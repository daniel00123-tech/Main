...
class RankingAndBaselineTest(unittest.TestCase):
    def test_sorts_staff_by_red_then_amber_then_workload(self) -> None:
        rows = [
            {"staff_name": "One Amber Larger", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 8},
            {"staff_name": "One Red", "red_kpis": 1, "amber_kpis": 0, "total_open_workload": 1},
            {"staff_name": "All Green", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 12},
            {"staff_name": "One Amber Smaller", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 3},
        ]

        self.assertEqual(
            [row["staff_name"] for row in sorted(rows, key=staff_sort_key)],
            ["All Green", "One Amber Smaller", "One Amber Larger", "One Red"],
        )
...
class DashboardRenderingTest(unittest.TestCase):
    def test_renders_only_bigchange_dashboard_columns(self) -> None:
        report = {
            "run_timestamp": "2026-05-25T07:00:00+00:00",
            "report_date": "2026-05-25",
            "job_lookback_start": "2025-05-25",
