...
class ReportBuildTest(unittest.TestCase):
    def test_kpi_order_is_bigchange_only(self) -> None:
        self.assertEqual(
            [metric_key for metric_key, _label in KPI_ORDER],
            ["unallocated_jobs", "historic_jobs", "uninvoiced_jobs", "unactioned_jobs"],
        )

    def test_sorts_staff_by_fewest_red_then_amber_then_workload(self) -> None:
        today = dt.date.today()
        amber_day = today - dt.timedelta(days=10)
        green_day = today - dt.timedelta(days=2)
        client = FakeReportBigChangeClient(
            unallocated=[
                {
                    "CategoryName": "All Green",
                    "StatusId": 1,
                    "ResourceName": "",
                    "PlannedStart": "",
                    "CreatedDate": green_day.isoformat(),
                },
                {
                    "CategoryName": "Red Staff",
                    "StatusId": 1,
                    "ResourceName": "",
                    "PlannedStart": "",
            ...
            historic=[
                {
                    "CategoryName": "Amber Staff",
                    "StatusId": 2,
                    "ResourceName": "Engineer",
                    "PlannedStart": amber_day.isoformat(),
                }
            ],
        )
        ...
        self.assertEqual([row["staff_name"] for row in report["staff_rows"]], ["All Green", "Amber Staff", "Red Staff"])
        self.assertEqual(report["staff_rows"][0]["red_kpis"], 0)
        self.assertEqual(report["staff_rows"][0]["amber_kpis"], 0)
...
class BaselineTest(unittest.TestCase):
    def test_saves_baseline_with_bigchange_kpis_and_sales_only(self) -> None:
        ...
        self.assertEqual(set(baseline["staff"][0]["counts"]), {metric_key for metric_key, _label in KPI_ORDER})
...
class FakeReportBigChangeClient:
    def __init__(self, unallocated=None, historic=None, uninvoiced=None, unactioned=None, invoices=None) -> None:
        self._unallocated = unallocated or []
        self._historic = historic or []
        self._uninvoiced = uninvoiced or []
        self._unactioned = unactioned or []
        self._invoices = invoices or []

    def categories(self):
        return [{"Name": "All Green"}, {"Name": "Amber Staff"}, {"Name": "Red Staff"}]
