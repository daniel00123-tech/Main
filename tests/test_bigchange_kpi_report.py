...
class RankingAndBaselineTest(unittest.TestCase):
    def test_severity_uses_oldest_item_age(self) -> None:
        self.assertEqual(severity_for(0, 99), "green")
        self.assertEqual(severity_for(1, 9), "green")
        self.assertEqual(severity_for(1, 10), "amber")
        self.assertEqual(severity_for(1, 30), "amber")
        self.assertEqual(severity_for(1, 31), "red")

    def test_staff_ranking_prioritises_fewest_red_then_amber_then_workload(self) -> None:
        rows = [
            {"staff_name": "Red Owner", "red_kpis": 1, "amber_kpis": 0, "total_open_workload": 1},
            {"staff_name": "All Green Busy", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 8},
            {"staff_name": "Amber Owner", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 1},
            {"staff_name": "All Green Clear", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 0},
        ]

        ordered = [row["staff_name"] for row in sorted(rows, key=staff_ranking_key)]

        self.assertEqual(ordered, ["All Green Clear", "All Green Busy", "Amber Owner", "Red Owner"])

    def test_saves_baseline_with_bigchange_kpis_only(self) -> None:
        report = {
    ...
    def test_rendered_dashboard_has_requested_bigchange_columns_only(self) -> None:
        report = {
            "run_timestamp": "2026-05-25T07:00:00+00:00",
            "report_date": "2026-05-25",
            "job_lookback_start": "2025-05-25",
