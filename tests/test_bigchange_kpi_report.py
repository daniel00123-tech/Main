...
class RankingAndBaselineTest(unittest.TestCase):
    def test_ranks_staff_by_red_then_amber_then_open_workload(self) -> None:
        rows = [
            {"staff_name": "Red", "red_kpis": 1, "amber_kpis": 0, "total_open_workload": 1},
            {"staff_name": "Green Busy", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 8},
            {"staff_name": "Amber", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 1},
            {"staff_name": "Green Clear", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 0},
        ]

        rows.sort(key=staff_ranking_key)

        self.assertEqual([row["staff_name"] for row in rows], ["Green Clear", "Green Busy", "Amber", "Red"])

    def test_saves_baseline_with_requested_bigchange_kpis_only(self) -> None:
        report = {
