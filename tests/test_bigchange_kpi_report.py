...
class RankingAndBaselineTest(unittest.TestCase):
    def test_ranks_staff_by_severity_then_workload(self) -> None:
        rows = [
            {"staff_name": "Red Owner", "red_kpis": 1, "amber_kpis": 0, "total_open_workload": 1},
            {"staff_name": "Heavy Green", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 9},
            {"staff_name": "Amber Owner", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 1},
            {"staff_name": "Light Green", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 2},
        ]

        self.assertEqual(
            [row["staff_name"] for row in sorted(rows, key=staff_ranking_key)],
            ["Light Green", "Heavy Green", "Amber Owner", "Red Owner"],
        )
    ...
    def test_saves_baseline_with_bigchange_kpis_only(self) -> None:
        ...
        self.assertEqual(set(baseline["staff"][0]["counts"]), {
            "unallocated_jobs",
            "historic_jobs",
            "uninvoiced_jobs",
            "unactioned_jobs",
    ...
    def test_dashboard_contains_requested_columns_only(self) -> None:
        report = {
            "run_timestamp": "2026-05-25T07:00:00+00:00",
            "report_date": "2026-05-25",
            "job_lookback_start": "2025-05-25",
