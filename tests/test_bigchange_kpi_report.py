...
class BaselineAndDashboardTest(unittest.TestCase):
    def test_saves_baseline_with_requested_bigchange_kpi_fields_only(self) -> None:
        report = {
    ...
    def test_dashboard_has_requested_columns_only(self) -> None:
        report = {
            "run_timestamp": "2026-05-25T07:00:00+00:00",
            "report_date": "2026-05-25",
            "job_lookback_start": "2025-05-25",
        ...
        self.assertIn("Unallocated Jobs", rendered)
        self.assertIn("Historic Jobs", rendered)
        self.assertIn("Uninvoiced Jobs", rendered)
        self.assertIn("Unactioned Jobs", rendered)
        self.assertIn("May sales", rendered)
        self.assertNotIn("Freshdesk", rendered)
        self.assertNotIn("Overall Score", rendered)


class ReportBuildTest(unittest.TestCase):
    def test_sorts_staff_best_to_worst_by_requested_ranking(self) -> None:
        today = dt.date.today()
        amber_date = today - dt.timedelta(days=12)
        red_date = today - dt.timedelta(days=31)
        client = FakeBigChangeClient(
            categories=[
                {"Name": "All Green"},
                {"Name": "Busy Green"},
            ...
            jobs={
                "unallocated": [
                    ...
                    {
                        "CategoryName": "Amber One",
                        "StatusId": 1,
                        "CreatedDate": amber_date.isoformat(),
                    },
                    {
                        "CategoryName": "Red One",
                        "StatusId": 1,
                        "CreatedDate": red_date.isoformat(),
                    },
                ]
            },
        )
        ...
        self.assertEqual(
            ...
            ["All Green", "Busy Green", "Amber One", "Red One"],
        )


class FakeBigChangeClient:
    def __init__(self, invoices=None, activities=None, categories=None, jobs=None) -> None:
    ...
    def jobslist(self, params):
        ...
        if params.get("ClientStatusId") == -34:
            return self._jobs.get("uninvoiced", [])
        if params.get("Unactioned") == 1:
            return self._jobs.get("unactioned", [])
        return []
