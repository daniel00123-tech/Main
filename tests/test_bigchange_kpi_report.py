...
class DashboardAndBaselineTest(unittest.TestCase):
    def test_staff_rank_key_sorts_by_red_then_amber_then_workload(self) -> None:
        rows = [
            {"staff_name": "Worst Red", "red_kpis": 1, "amber_kpis": 0, "total_open_workload": 0},
            {"staff_name": "Best Green", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 0},
            {"staff_name": "More Amber", "red_kpis": 0, "amber_kpis": 2, "total_open_workload": 1},
            {"staff_name": "Less Work", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 2},
            {"staff_name": "More Work", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 5},
        ]

        self.assertEqual(
            [row["staff_name"] for row in sorted(rows, key=staff_rank_key)],
            ["Best Green", "Less Work", "More Work", "More Amber", "Worst Red"],
        )

    def test_rendered_dashboard_has_requested_bigchange_only_columns(self) -> None:
        report = sample_report()

        rendered = render_html(report)

        self.assertIn("<th>Unallocated Jobs</th>", rendered)
        self.assertIn("<th>Historic Jobs</th>", rendered)
        self.assertIn("<th>Uninvoiced Jobs</th>", rendered)
        self.assertIn("<th>Unactioned Jobs</th>", rendered)
        self.assertIn("<th>May sales</th>", rendered)
        self.assertNotIn("Freshdesk", rendered)
    ...
    def test_saves_baseline_with_requested_kpi_fields(self) -> None:
        ...
        self.assertEqual(set(row["counts"]), {"unallocated_jobs", "historic_jobs", "uninvoiced_jobs", "unactioned_jobs"})
