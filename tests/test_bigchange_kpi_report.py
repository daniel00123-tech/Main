...
class ReportBaselineTest(unittest.TestCase):
    def test_saves_baseline_with_bigchange_only_kpi_fields(self) -> None:
        report = {
        ...
        self.assertEqual(set(staff_row["counts"]), {"unallocated_jobs", "historic_jobs", "uninvoiced_jobs", "unactioned_jobs"})
    ...
    def test_sorts_staff_best_to_worst_by_red_amber_then_workload(self) -> None:
        ...
        client = FakeReportBigChangeClient(
            unallocated_rows=[
                {
                    "JobCategoryName": "Red One",
                    "StatusId": 1,
                    "CreatedDate": (today - dt.timedelta(days=31)).isoformat(),
                },
                {
                    "JobCategoryName": "Amber Low",
                    "StatusId": 1,
                    "CreatedDate": (today - dt.timedelta(days=10)).isoformat(),
                },
                {
                    "JobCategoryName": "Amber High",
                    "StatusId": 1,
                    "CreatedDate": (today - dt.timedelta(days=10)).isoformat(),
                },
                {
        ...
        self.assertEqual(
            ...
            ["All Green", "Amber Low", "Amber High", "Red One"],
        )
...
class FakeReportBigChangeClient:
    def __init__(self, unallocated_rows) -> None:
