...
class ReportRankingTest(unittest.TestCase):
    def test_sorts_staff_by_reds_then_ambers_then_open_workload(self) -> None:
        today = dt.date.today()
        client = FakeReportClient(
            categories=["Clean Staff", "Workload Staff", "Amber Staff", "Red Staff"],
            unallocated_jobs=[
                {
                    "CategoryName": "Red Staff",
                    "StatusId": 1,
                    "CreatedDate": (today - dt.timedelta(days=31)).isoformat(),
                },
                {
                    "CategoryName": "Amber Staff",
                    "StatusId": 1,
                    "CreatedDate": (today - dt.timedelta(days=12)).isoformat(),
                },
                {
                    "CategoryName": "Workload Staff",
                    "StatusId": 1,
        ...
        self.assertEqual(
            ...
            ["Clean Staff", "Workload Staff", "Amber Staff", "Red Staff"],
        )


class BaselineTest(unittest.TestCase):
    def test_saves_baseline_with_job_kpi_statuses_and_sales(self) -> None:
        report = {
...
class FakeReportClient:
    def __init__(self, categories, unallocated_jobs) -> None:
        self._categories = categories
        self._unallocated_jobs = unallocated_jobs
