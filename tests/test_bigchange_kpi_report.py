...
class ReportShapeTest(unittest.TestCase):
    def test_sorts_staff_by_red_then_amber_then_open_workload(self) -> None:
        report = build_report(FakeKpiBuildBigChangeClient())

        self.assertEqual(
            [row["staff_name"] for row in report["staff_rows"]],
            ["All Green", "Busy Green", "Amber Staff", "Red Staff"],
        )

    def test_html_contains_bigchange_only_kpi_columns(self) -> None:
        report = build_report(FakeKpiBuildBigChangeClient())

        dashboard_html = render_html(report)

        self.assertIn("Unallocated Jobs", dashboard_html)
        self.assertIn("Historic Jobs", dashboard_html)
        self.assertIn("Uninvoiced Jobs", dashboard_html)
        self.assertIn("Unactioned Jobs", dashboard_html)
        self.assertIn(f"{dt.date.today().strftime('%B')} sales", dashboard_html)
        self.assertNotIn("Freshdesk", dashboard_html)
        self.assertNotIn("Overall Score", dashboard_html)
        self.assertNotIn("good", dashboard_html.lower())
        self.assertNotIn("medium", dashboard_html.lower())
        self.assertNotIn("high", dashboard_html.lower())

    def test_saves_baseline_with_job_kpi_fields_and_sales_only(self) -> None:
        report = {
        ...
        row = baseline["staff"][0]
        self.assertEqual(row["counts"]["historic_jobs"], 1)
        self.assertEqual(row["statuses"]["historic_jobs"], "amber")
        self.assertEqual(row["oldest_age_days"]["historic_jobs"], 12)
        self.assertEqual(row["current_month_sales"], 123.45)
        self.assertNotIn("open_freshdesk_tickets", row["counts"])
        self.assertNotIn("freshdesk_ticket_count", row)
...
class FakeKpiBuildBigChangeClient:
    staff = ("Red Staff", "Amber Staff", "Busy Green", "All Green")

    def categories(self):
        return [{"Name": name} for name in self.staff]
    ...
    def jobslist(self, params, page_size=500):
        ...
        today = dt.date.today()
        rows = [
            self._unallocated_row("Red Staff", today - dt.timedelta(days=31)),
            self._unallocated_row("Amber Staff", today - dt.timedelta(days=10)),
        ]
        rows.extend(self._unallocated_row("Busy Green", today - dt.timedelta(days=1)) for _ in range(5))
        return rows
    ...
    @staticmethod
    def _unallocated_row(category, created):
        return {
