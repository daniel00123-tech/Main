...
class BaselineAndDashboardTest(unittest.TestCase):
    def test_saves_baseline_with_only_bigchange_kpi_fields(self) -> None:
        report = {
        ...
        staff_row = baseline["staff"][0]
        self.assertEqual(staff_row["current_month_sales"], 123.45)
        self.assertEqual(staff_row["counts"]["historic_jobs"], 1)
        self.assertEqual(staff_row["statuses"]["historic_jobs"], "amber")
        self.assertNotIn("open_freshdesk_tickets", staff_row["counts"])
        self.assertNotIn("freshdesk_ticket_count", staff_row)
        self.assertNotIn("overall_score", staff_row)

    def test_dashboard_uses_requested_bigchange_columns_only(self) -> None:
        report = {
            "run_timestamp": "2026-05-25T07:00:00+00:00",
            "report_date": "2026-05-25",
            "job_lookback_start": "2025-05-25",
            ...
            "staff_rows": [
                {
                    ...
                    "metrics": {
                        "unallocated_jobs": {"count": 0, "status": "green", "oldest_age_days": 0},
                        "historic_jobs": {"count": 1, "status": "amber", "oldest_age_days": 12},
                        "uninvoiced_jobs": {"count": 1, "status": "red", "oldest_age_days": 31},
                        "unactioned_jobs": {"count": 0, "status": "green", "oldest_age_days": 0},
                    },
                    "current_month_sales": 123.45,
        ...
        self.assertNotIn("Open Freshdesk Tickets", rendered)
