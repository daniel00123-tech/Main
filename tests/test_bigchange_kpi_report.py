...
class BaselineTest(unittest.TestCase):
    def test_saves_baseline_with_requested_bigchange_kpi_fields(self) -> None:
        report = {
...
class BuildReportRankingTest(unittest.TestCase):
    def test_sorts_staff_by_fewest_reds_then_ambers_then_workload(self) -> None:
        today = dt.date.today()
        client = FakeBuildReportClient(
            categories=["Green Staff", "Busy Green", "Amber Staff", "Red Staff"],
            unallocated_rows=[
                job_row("Busy Green", status_id=1, created=today - dt.timedelta(days=1)),
                job_row("Busy Green", status_id=1, created=today - dt.timedelta(days=1)),
                job_row("Busy Green", status_id=1, created=today - dt.timedelta(days=1)),
                job_row("Amber Staff", status_id=1, created=today - dt.timedelta(days=10)),
                job_row("Red Staff", status_id=1, created=today - dt.timedelta(days=31)),
            ],
        )
        ...
        self.assertEqual(
            ...
            ["Green Staff", "Busy Green", "Amber Staff", "Red Staff"],
        )
        self.assertEqual(report["total_red_kpis"], 1)
        self.assertEqual(report["total_amber_kpis"], 1)


def job_row(staff_name: str, status_id: int, created: dt.date) -> dict:
    return {
        "CategoryName": staff_name,
        "StatusId": status_id,
...
class FakeBuildReportClient:
    def __init__(self, categories, unallocated_rows) -> None:
        self._categories = categories
        self._unallocated_rows = unallocated_rows

    def categories(self):
        return [{"Name": name} for name in self._categories]
