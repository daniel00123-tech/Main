...
class RankingAndBaselineTest(unittest.TestCase):
    def test_staff_ranking_prefers_fewer_reds_then_ambers_then_workload(self) -> None:
        rows = [
            {"staff_name": "More Workload", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 4},
            {"staff_name": "One Amber", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 0},
            {"staff_name": "All Green", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 0},
            {"staff_name": "One Red", "red_kpis": 1, "amber_kpis": 0, "total_open_workload": 0},
        ]

        rows.sort(key=staff_ranking_key)

        self.assertEqual([row["staff_name"] for row in rows], ["All Green", "More Workload", "One Amber", "One Red"])
