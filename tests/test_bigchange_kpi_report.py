...
class ScoreAndBaselineTest(unittest.TestCase):
    def test_sorts_staff_best_to_worst_by_red_amber_then_workload(self) -> None:
        rows = [
            {"staff_name": "High Workload", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 12},
            {"staff_name": "Amber Owner", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 1},
            {"staff_name": "Red Owner", "red_kpis": 1, "amber_kpis": 0, "total_open_workload": 0},
            {"staff_name": "All Green", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 0},
        ]

        sorted_rows = sorted(rows, key=staff_sort_key)

        self.assertEqual([row["staff_name"] for row in sorted_rows], ["All Green", "High Workload", "Amber Owner", "Red Owner"])

    def test_saves_baseline_with_counts_statuses_ages_and_sales(self) -> None:
        report = {
