...
class BuildReportRankingTest(unittest.TestCase):
    def test_sorts_staff_by_red_amber_then_workload(self) -> None:
        client = FakeReportClient(dt.date.today())

        report = build_report(client)

        self.assertEqual(
            [row["staff_name"] for row in report["staff_rows"]],
            ["Clean Staff", "Busy Green", "Amber Staff", "Red Staff"],
        )
...
class BaselineTest(unittest.TestCase):
    def test_saves_baseline_with_bigchange_kpi_counts_statuses_and_sales(self) -> None:
        report = {
...
class FakeReportClient:
    ...
    def jobslist(self, params, page_size=500):
        if params.get("Unallocated"):
            return [
                {
                    "CategoryName": "Clean Staff",
                    "StatusId": 1,
                    "CreatedDate": (self.today - dt.timedelta(days=1)).isoformat(),
                },
                {
                ...
                {
                    "CategoryName": "Ryan Barrett",
                    "StatusId": 1,
                    "CreatedDate": (self.today - dt.timedelta(days=60)).isoformat(),
                },
            ]
            ...
            return [
                {
                    ...
                    "PlannedStart": (self.today - dt.timedelta(days=31)).isoformat(),
                }
            ]
            ...
            return [
                {
                    ...
                    "StatusId": 12,
                    "Actioned": "No",
                    "CompletedDate": (self.today - dt.timedelta(days=10)).isoformat(),
                }
            ]
        return []
