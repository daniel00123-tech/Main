...
class RankingAndBaselineTest(unittest.TestCase):
    def test_sorts_staff_by_red_then_amber_then_workload(self) -> None:
        rows = [
            {"staff_name": "Red One", "red_kpis": 1, "amber_kpis": 0, "total_open_workload": 1},
            {"staff_name": "All Green Busy", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 9},
            {"staff_name": "Amber Light", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 1},
            {"staff_name": "All Green Clear", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 0},
        ]

        sorted_names = [row["staff_name"] for row in sorted(rows, key=staff_sort_key)]

        self.assertEqual(sorted_names, ["All Green Clear", "All Green Busy", "Amber Light", "Red One"])
