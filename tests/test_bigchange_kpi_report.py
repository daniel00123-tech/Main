...
class ReportRankingTest(unittest.TestCase):
    def test_sorts_staff_by_fewest_red_then_amber_then_workload(self) -> None:
        ...
        client = FakeReportClient(
            categories=["All Green", "Amber Owner", "Red Owner"],
            historic_rows=[
                {
                    "CategoryName": "Amber Owner",
                    "StatusId": 1,
                    "ResourceName": "Engineer",
                    "PlannedStart": (today - dt.timedelta(days=12)).isoformat(),
                },
                {
                    "CategoryName": "Red Owner",
                    "StatusId": 1,
                    "ResourceName": "Engineer",
                    "PlannedStart": (today - dt.timedelta(days=35)).isoformat(),
                },
            ],
        )
        ...
        self.assertEqual([row["staff_name"] for row in report["staff_rows"]], ["All Green", "Amber Owner", "Red Owner"])
...
class FakeReportClient:
    def __init__(self, categories, historic_rows) -> None:
        self._categories = categories
        self._historic_rows = historic_rows
