...
class BaselineAndRenderingTest(unittest.TestCase):
    def test_saves_baseline_with_bigchange_kpis_and_sales_only(self) -> None:
        ...
        self.assertEqual(set(staff["counts"]), {"unallocated_jobs", "historic_jobs", "uninvoiced_jobs", "unactioned_jobs"})
        self.assertNotIn("open_freshdesk_tickets", staff["counts"])
        self.assertNotIn("overall_score", staff)

    def test_render_html_uses_requested_bigchange_columns(self) -> None:
        metric = {"count": 0, "status": "green", "oldest_age_days": 0}
        report = {
            "run_timestamp": "2026-06-28T07:00:00+00:00",
            "report_date": "2026-06-28",
            "job_lookback_start": "2025-06-28",
            "month_name": "June",
            "total_red_kpis": 0,
            "total_amber_kpis": 0,
            "staff_rows": [
                {
                    "staff_name": "Amy Bradley",
                    "metrics": {
                        "unallocated_jobs": metric,
                        "historic_jobs": metric,
                        "uninvoiced_jobs": metric,
                        "unactioned_jobs": metric,
                    },
        ...
        dashboard = render_html(report)

        self.assertIn("<th>Unallocated Jobs</th>", dashboard)
        self.assertIn("<th>Historic Jobs</th>", dashboard)
        self.assertIn("<th>Uninvoiced Jobs</th>", dashboard)
        self.assertIn("<th>Unactioned Jobs</th>", dashboard)
        self.assertIn("<th>June sales</th>", dashboard)
        self.assertNotIn("Freshdesk", dashboard)
        self.assertNotIn("Overall Score", dashboard)
