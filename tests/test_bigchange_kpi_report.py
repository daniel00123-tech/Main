...
class ReportBuildTest(unittest.TestCase):
    def test_sorts_staff_by_reds_ambers_then_open_workload(self) -> None:
        today = dt.date.today()
        client = FakeReportBigChangeClient(
            categories=["All Green", "Green With Work", "Red Owner"],
            unallocated_rows=[
                {
                    "CategoryName": "Green With Work",
                    "StatusId": 1,
                    "CreatedDate": (today - dt.timedelta(days=5)).isoformat(),
                    "ResourceName": "",
                },
                {
                    "CategoryName": "Red Owner",
                    "StatusId": 1,
                    "CreatedDate": (today - dt.timedelta(days=40)).isoformat(),
                    "ResourceName": "",
                },
            ],
        )
        ...
        self.assertEqual([row["staff_name"] for row in report["staff_rows"]], ["All Green", "Green With Work", "Red Owner"])
        self.assertEqual(report["staff_rows"][0]["total_open_workload"], 0)
        self.assertEqual(report["staff_rows"][2]["red_kpis"], 1)

    def test_renders_only_requested_kpi_columns(self) -> None:
        report = {
            "run_timestamp": "2026-06-01T07:00:00+00:00",
            "report_date": "2026-06-01",
            "job_lookback_start": "2025-06-01",
            "month_name": "June",
            "staff_rows": [
                {
...
class BaselineTest(unittest.TestCase):
    def test_saves_baseline_with_requested_kpi_counts_statuses_and_sales(self) -> None:
        report = {
...
class FakeReportBigChangeClient:
    def __init__(self, categories, unallocated_rows=None, historic_rows=None, uninvoiced_rows=None, unactioned_rows=None) -> None:
        self._categories = categories
        self._unallocated_rows = unallocated_rows or []
        self._historic_rows = historic_rows or []
        self._uninvoiced_rows = uninvoiced_rows or []
        self._unactioned_rows = unactioned_rows or []

    def categories(self):
        return [{"Name": category} for category in self._categories]

    def jobslist(self, params):
        if params.get("Unallocated"):
            return self._unallocated_rows
        if params.get("Allocated"):
            return self._historic_rows
        if params.get("ClientStatusId") == -34:
            return self._uninvoiced_rows
        if params.get("Unactioned"):
            return self._unactioned_rows
        return []
