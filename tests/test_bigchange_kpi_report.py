...
class RankingAndBaselineTest(unittest.TestCase):
    def test_sorts_staff_by_red_then_amber_then_open_workload(self) -> None:
        today = dt.date.today()
        client = FakeReportClient(
            categories=["All Green", "Busy Green", "Amber Person", "Red Person"],
            unallocated=[
                {
                    "CategoryName": "Busy Green",
                    "StatusId": 1,
                    "ResourceName": "",
                    "PlannedStart": "",
                    "CreatedDate": (today - dt.timedelta(days=1)).isoformat(),
                },
                {
                    "CategoryName": "Busy Green",
                    "StatusId": 3,
                    "ResourceName": "",
                    "PlannedStart": "",
                    "CreatedDate": (today - dt.timedelta(days=2)).isoformat(),
                },
            ],
            historic=[
                {
                    "CategoryName": "Amber Person",
                    "StatusId": 2,
                    "ResourceName": "Engineer One",
                    "PlannedStart": (today - dt.timedelta(days=15)).isoformat(),
                },
                {
                    "CategoryName": "Red Person",
                    "StatusId": 2,
                    "ResourceName": "Engineer Two",
                    "PlannedStart": (today - dt.timedelta(days=40)).isoformat(),
                },
            ],
            uninvoiced=[],
            unactioned=[],
        ...
        self.assertEqual(
            ...
            ["All Green", "Busy Green", "Amber Person", "Red Person"],
        )

    def test_saves_baseline_with_requested_bigchange_only_fields(self) -> None:
        report = {
...
class FakeReportClient(FakeBigChangeClient):
    def __init__(self, categories, unallocated, historic, uninvoiced, unactioned) -> None:
        super().__init__(invoices=[], activities={})
        self._categories = categories
        self._unallocated = unallocated
        self._historic = historic
        self._uninvoiced = uninvoiced
        self._unactioned = unactioned
