...
class BigChangeApiTest(unittest.TestCase):
    ...
    def test_treats_bigchange_placeholder_dates_as_blank(self) -> None:
        self.assertIsNone(parse_date("0001-01-01"))
        self.assertIsNone(parse_date("0001-01-01 00:00:00"))
...
class RankingAndRenderTest(unittest.TestCase):
    def test_sorts_staff_by_red_then_amber_then_open_workload(self) -> None:
        rows = [
            {"staff_name": "Red Staff", "red_kpis": 1, "amber_kpis": 0, "total_open_workload": 0},
            {"staff_name": "Amber Staff", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 0},
            {"staff_name": "Busy Green", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 5},
            {"staff_name": "Clear Green", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 0},
        ]

        self.assertEqual(
            [row["staff_name"] for row in sorted(rows, key=staff_sort_key)],
            ["Clear Green", "Busy Green", "Amber Staff", "Red Staff"],
        )

    def test_renders_requested_bigchange_columns_only(self) -> None:
        metrics = {
            ...
            "unactioned_jobs": {"count": 2, "status": "red", "oldest_age_days": 31},
        }
        report = {
            "run_timestamp": "2026-06-02T07:00:00+00:00",
            "report_date": "2026-06-02",
            "job_lookback_start": "2025-06-02",
            "month_name": "June",
            "total_red_kpis": 1,
            "total_amber_kpis": 1,
            "staff_rows": [
...
class BaselineTest(unittest.TestCase):
    def test_saves_baseline_with_requested_kpi_status_and_sales_fields(self) -> None:
        report = {
        ...
        self.assertEqual(staff["oldest_age_days"]["historic_jobs"], 12)
