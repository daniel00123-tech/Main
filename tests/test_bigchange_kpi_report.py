...
class BaselineTest(unittest.TestCase):
    def test_saves_bigchange_only_baseline_fields(self) -> None:
        metrics = {
            metric_key: {"count": idx, "status": "green", "oldest_age_days": idx + 1}
            for idx, (metric_key, _label) in enumerate(KPI_ORDER)
        }
        report = {
            "run_timestamp": "2026-07-12T07:00:00+00:00",
            "report_date": "2026-07-12",
            "job_lookback_start": "2025-07-12",
            "staff_rows": [
                {
        ...
        self.assertEqual(staff_row["staff_name"], "Sharon Mannion")
        self.assertEqual(set(staff_row["counts"]), {metric_key for metric_key, _label in KPI_ORDER})
