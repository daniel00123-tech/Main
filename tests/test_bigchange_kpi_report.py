...
class RankingAndBaselineTest(unittest.TestCase):
    def test_ranks_by_fewest_red_then_amber_then_workload(self) -> None:
        rows = [
            {"staff_name": "Needs Attention", "red_kpis": 1, "amber_kpis": 0, "total_open_workload": 1},
            {"staff_name": "Amber Team", "red_kpis": 0, "amber_kpis": 1, "total_open_workload": 1},
            {"staff_name": "All Green Busier", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 4},
            {"staff_name": "All Green Clear", "red_kpis": 0, "amber_kpis": 0, "total_open_workload": 0},
        ]

        ranked = [row["staff_name"] for row in sorted(rows, key=staff_ranking_key)]

        self.assertEqual(ranked, ["All Green Clear", "All Green Busier", "Amber Team", "Needs Attention"])
