...
class FakeReportBigChangeClient:
    ...
    def jobslist(self, params):
        if params.get("Unallocated"):
            return [
                *[
                    self._job("Green Workload", status=1, created_days_ago=1)
                    for _ in range(5)
                ],
                self._job("Amber Staff", status=1, created_days_ago=10),
            ]
        if params.get("Unactioned"):
            return [
                self._job("Red Staff", status=12, completed_days_ago=31, actioned="No"),
            ]
        return []
    ...
    def _job(
        ...
        return {
            ...
            "StatusId": status,
            "CreatedDate": (self.today - dt.timedelta(days=created_days_ago)).isoformat(),
            "CompletedDate": (self.today - dt.timedelta(days=completed_days_ago)).isoformat(),
            "Actioned": actioned,
        }
