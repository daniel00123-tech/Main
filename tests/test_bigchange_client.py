...
def test_api_key_mode_uses_legacy_jobslist_request() -> None:
    ...
    config = make_legacy_config(lookahead_days=21)
    jobs = BigChangeClient(config).iter_completed_jobs(limit=2)

    assert [job["JobId"] for job in jobs] == [1, 2]
    request = responses.calls[0].request
    query = parse_qs(urlparse(request.url).query)
    assert query["action"] == ["JobsList"]
    assert query["Start"] == [config.legacy_start_date]
    assert query["End"] == [config.legacy_end_date]
    assert query["Page"] == ["0"]
...
@responses.activate
def test_api_key_mode_treats_no_results_response_as_empty_page() -> None:
    responses.add(
        responses.GET,
        "https://webservice.example.test/v01/services.ashx",
        json={"Code": 0, "Result": "No results"},
    )

    jobs = BigChangeClient(make_legacy_config()).iter_completed_jobs()
