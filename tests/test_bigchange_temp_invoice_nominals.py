...
class FakeBigChangeClient:
    def __init__(
        ...
        verified_doc_overrides: dict[str, Any] | None = None,
        verified_tax_code: Any | None = None,
    ) -> None:
        self.rows = rows
        ...
        self.verified_doc_overrides = verified_doc_overrides or {}
    ...
    def test_rejects_synchronised_documents(self) -> None:
        for doc in (
            ...
            with self.subTest(doc=doc):
                ...
                self.assertFalse(processable)
                self.assertRegex(reason.lower(), r"sync|synchron")
...
class TempInvoiceNominalCorrectorTest(unittest.TestCase):
    ...
    def test_uses_job_reference_when_job_id_is_missing(self) -> None:
        doc = {
            "DocId": "D1",
            "Reference": "TEMP-100",
            "DocumentType": "Invoice",
            "JobReference": "JOB-1",
            "FinancialLines": [{"UnitPrice": "1", "Quantity": "1", "TaxCode": "T1", "TaxRate": "20"}],
        }
        client = FakeBigChangeClient(
            rows=[{"InvoiceType": "SI", "Reference": "TEMP-100", "InvoiceId": "D1"}],
            docs_by_ref={"TEMP-100": doc},
            docs_by_id={"D1": doc},
            jobs_by_id={},
            jobs_by_ref={"JOB-1": {"JobId": "J1", "Type": "Electrical", "Description": "PPM service"}},
        )
        ...
        self.assertEqual(report.invoices_updated, 1)
        self.assertEqual(client.added_lines[0]["JobId"], "J1")
        self.assertEqual(client.created_items[0]["NominalCode"], "2101")

    def test_hydrates_first_group_job_before_classification(self) -> None:
        doc = {
            "DocId": "D1",
            "Reference": "TEMP-100",
            "DocumentType": "Invoice",
    ...
    def test_verification_rejects_wrong_document_id(self) -> None:
        doc = {
            "DocId": "D1",
            "Reference": "TEMP-100",
            "DocumentType": "Invoice",
        ...
        client = FakeBigChangeClient(
            ...
            docs_by_id={"D1": doc},
            jobs_by_id={"J1": {"JobId": "J1", "Type": "Fire", "Description": "Call Out"}},
            verified_doc_overrides={"DocId": "D2"},
        )
        ...
        self.assertIn("DocId D2", report.failures[0]["reason"])
