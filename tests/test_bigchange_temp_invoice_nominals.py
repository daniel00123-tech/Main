...
class SafetyFilterTest(unittest.TestCase):
    ...
    def test_rejects_purchase_invoice_even_if_type_contains_invoice(self) -> None:
        processable, reason = document_is_processable({"DocumentType": "Purchase Invoice"})

        self.assertFalse(processable)
        self.assertIn("Purchase Invoice", reason)

    def test_rejects_synchronised_or_status_rejected_documents(self) -> None:
        cases = (
            ({"DocumentType": "Invoice", "IsSynchronised": "true"}, "IsSynchronised"),
            ({"DocumentType": "Invoice", "SynchronisedDate": "2026-05-19"}, "SynchronisedDate"),
            ({"DocumentType": "Invoice", "DocumentStatus": "Rejected"}, "Rejected"),
        )

        for doc, reason_text in cases:
...
class TempInvoiceNominalCorrectorTest(unittest.TestCase):
    ...
    def test_scans_duplicate_rows_for_same_temp_invoice_once(self) -> None:
        doc = {
            "DocId": "D1",
            "Reference": "TEMP-100",
            "DocumentType": "Invoice",
            "JobId": "J1",
            "FinancialLines": [{"UnitPrice": "1", "Quantity": "1", "NominalCode": "2205"}],
        }
        client = FakeBigChangeClient(
            rows=[
                {"InvoiceType": "SI", "Reference": "TEMP-100", "InvoiceId": "D1"},
                {"InvoiceType": "SI", "Reference": "TEMP-100", "InvoiceId": "D1-LINE"},
            ],
            docs_by_ref={"TEMP-100": doc},
            docs_by_id={"D1": doc},
    ...
    def test_resolves_first_group_job_by_reference_when_group_has_no_job_id(self) -> None:
        doc = {
            "DocId": "D1",
            "Reference": "TEMP-100",
            "DocumentType": "Invoice",
            "GroupId": "G1",
            "FinancialLines": [{"UnitPrice": "1", "Quantity": "1", "TaxCode": "T1", "NominalCode": ""}],
        }
        client = FakeBigChangeClient(
            rows=[{"InvoiceType": "SI", "Reference": "TEMP-100", "InvoiceId": "D1"}],
            docs_by_ref={"TEMP-100": doc},
            docs_by_id={"D1": doc},
            jobs_by_id={},
            jobs_by_ref={"JOB-1": {"JobId": "J1", "JobReference": "JOB-1", "Type": "Fire", "Description": "PPM service"}},
            group_jobs_by_id={"G1": [{"JobReference": "JOB-1"}]},
        )
