...
class FakeBigChangeClient:
    def __init__(
        ...
        verified_doc_id: str | None = None,
        verified_tax_code: str | None = None,
    ) -> None:
        self.rows = rows
        ...
        self.verified_doc_id = verified_doc_id
        self.verified_tax_code = verified_tax_code
...
class SafetyFilterTest(unittest.TestCase):
    ...
    def test_rejects_missing_purchase_and_synchronised_documents(self) -> None:
        cases = (
            ({"DocId": "D1"}, "missing"),
            ({"DocumentType": "Purchase Invoice"}, "Purchase Invoice"),
            ({"DocumentType": "Invoice", "InvoiceType": "PO"}, "PO"),
            ({"DocumentType": "Invoice", "SynchronisedDate": "2026-05-21"}, "SynchronisedDate"),
            ({"DocumentType": "Invoice", "IsExported": True}, "IsExported"),
        )

        for doc, expected_reason in cases:
...
class TempInvoiceNominalCorrectorTest(unittest.TestCase):
    ...
    def test_retries_financial_doc_lookup_by_id_when_reference_lookup_fails(self) -> None:
        doc = {
            "DocId": "D1",
            "Reference": "TEMP-100",
            "DocumentType": "Invoice",
        ...
        client = FakeBigChangeClient(
            rows=[{"InvoiceType": "SI", "Reference": "TEMP-100", "InvoiceId": "D1"}],
            docs_by_ref={"TEMP-100": None},
            docs_by_id={"D1": doc},
            jobs_by_id={"J1": {"JobId": "J1", "Type": "Grounds", "Description": "Project works"}},
        )

        report = TempInvoiceNominalCorrector(client).run()

        self.assertEqual(report.invoices_updated, 1)
        self.assertEqual(client.generated_docs[0]["DocId"], "D1")

    def test_resolves_job_by_reference_and_hydrates_group_job(self) -> None:
        direct_ref_doc = {
            "DocId": "D1",
            "Reference": "TEMP-100",
            "DocumentType": "Invoice",
        ...
        client = FakeBigChangeClient(
            rows=[
                {"InvoiceType": "SI", "Reference": "TEMP-100", "InvoiceId": "D1"},
                {"InvoiceType": "SI", "Reference": "TEMP-200", "InvoiceId": "D2"},
            ],
            docs_by_ref={"TEMP-100": direct_ref_doc, "TEMP-200": group_doc},
            docs_by_id={"D1": direct_ref_doc, "D2": group_doc},
            jobs_by_id={
                "J2": {"JobId": "J2", "Type": "Cleaning", "Description": "Scheduled service"},
            },
            jobs_by_ref={
                "JR1": {"JobId": "J1", "Type": "Electrical", "Description": "Repair fault"},
            },
        ...
        self.assertEqual(report.invoices_updated, 2)
        self.assertEqual(client.generated_docs[0]["JobId"], "J1")
        self.assertEqual(client.added_lines[0]["NominalCode"], "2001")
        self.assertEqual(client.generated_docs[1]["JobId"], "J2")
        self.assertEqual(client.added_lines[1]["NominalCode"], "2104")
    ...
    def test_accepts_tax_code_normalisation_when_tax_rate_is_preserved(self) -> None:
        ...
        client = FakeBigChangeClient(
            ...
            docs_by_ref={"TEMP-100": doc},
            docs_by_id={"D1": doc},
            jobs_by_id={"J1": {"JobId": "J1", "Type": "Fire", "Description": "Service"}},
            verified_tax_code="20%",
        )
    ...
    def test_fails_verification_when_doc_id_changes(self) -> None:
        doc = {
            "DocId": "D1",
            "Reference": "TEMP-100",
            "DocumentType": "Invoice",
