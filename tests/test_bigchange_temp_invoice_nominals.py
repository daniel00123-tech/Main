...
class SafetyFilterTest(unittest.TestCase):
    ...
    def test_rejects_ambiguous_purchase_and_missing_document_types(self) -> None:
        cases = (
    ...
    def test_rejects_synchronised_or_exported_documents(self) -> None:
        cases = (
            ...
            {"DocumentType": "Invoice", "SyncStatus": "Synced"},
            {"DocumentType": "Invoice", "ExportedDate": "2026-05-21"},
            {"DocumentType": "Invoice", "SageId": "S-1"},
        )
    ...
    def test_allows_unsynchronised_markers(self) -> None:
        processable, reason = document_is_processable(
            {"DocumentType": "Invoice", "IsSynchronised": False, "SyncStatus": "Not Synchronised"}
        )
...
class TempInvoiceNominalCorrectorTest(unittest.TestCase):
    ...
    def test_deduplicates_temp_rows_by_reference(self) -> None:
        doc = {
            ...
            "DocumentType": "Invoice",
            "JobId": "J1",
            "FinancialLines": [{"UnitPrice": "1", "Quantity": "1", "TaxCode": "T1", "TaxRate": "20", "NominalCode": "9999"}],
        }
        client = FakeBigChangeClient(
            rows=[
    ...
    def test_resolves_job_by_job_reference(self) -> None:
        doc = {
            "DocId": "D1",
            "Reference": "TEMP-100",
            "DocumentType": "Invoice",
        ...
        client = FakeBigChangeClient(
            ...
            docs_by_id={"D1": doc},
            jobs_by_id={},
            jobs_by_ref={"JOB-1": {"JobId": "J1", "Reference": "JOB-1", "Type": "Cleaning", "Description": "PPM"}},
        )

        report = TempInvoiceNominalCorrector(client).run()

        self.assertEqual(report.failures, [])
        self.assertEqual(client.created_items[0]["NominalCode"], "2104")
    ...
    def test_resolves_first_group_job_through_job_endpoint(self) -> None:
        ...
        client = FakeBigChangeClient(
            ...
            docs_by_ref={"TEMP-100": doc},
            docs_by_id={"D1": doc},
            jobs_by_id={"J1": {"JobId": "J1", "Type": "Grounds", "Description": "Reactive repair"}},
            group_jobs_by_id={"G1": [{"JobId": "J1", "Type": "Unknown", "Description": "Unknown"}]},
        )

        report = TempInvoiceNominalCorrector(client).run()

        self.assertEqual(report.failures, [])
        self.assertEqual(client.created_items[0]["NominalCode"], "2005")

    def test_skips_synchronised_full_document(self) -> None:
        doc = {
            "DocId": "D1",
            "Reference": "TEMP-100",
            "DocumentType": "Invoice",
    ...
    def test_accepts_tax_code_normalisation_when_tax_rate_is_preserved(self) -> None:
        class VatNormalisingClient(FakeBigChangeClient):
            def generate_financial_doc_for_job(self, params: dict[str, Any]) -> dict[str, Any]:
                payload = super().generate_financial_doc_for_job(params)
                doc_id = str(params["DocId"])
                self.verified_docs_by_id[doc_id]["FinancialLines"][0]["TaxCode"] = "VAT 20"
                return payload
    ...
    def test_verification_fails_if_doc_id_changes(self) -> None:
        class WrongDocClient(FakeBigChangeClient):
            def generate_financial_doc_for_job(self, params: dict[str, Any]) -> dict[str, Any]:
                payload = super().generate_financial_doc_for_job(params)
                doc_id = str(params["DocId"])
                self.verified_docs_by_id[doc_id]["DocId"] = "D2"
                return payload
        ...
        self.assertIn("document id", report.failures[0]["reason"])
