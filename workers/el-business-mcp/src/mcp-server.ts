...
import { requireCapability } from "./rbac/guard";
...
export function createElBusinessMcpServer(env: Env): McpServer {
  ...
  server.registerTool(
    ...
    async () => {
      try {
        await requireCapability(env, "knowledge.company.read", { resource: "database_summary" });
        const tables = await getDatabaseSummary(
          env.EL_BUSINESS_DATA,
          EL_STRUCTURED_DATA_CONFIG.summary
        );
  ...
  server.registerTool(
    ...
    async ({ sql, limit = 100 }) => {
      try {
        await requireCapability(env, "knowledge.company.read", { resource: "query_business_data" });
      } catch (error) {
        return {
