...
import { authorize, requireCalendar, requireCapability, requireMailbox } from "../rbac/guard";
import { AuthorizationError } from "../rbac/errors";
import { filterKnowledgeItems } from "../rbac/knowledge";
import { getRequestActor } from "../rbac/context";
...
export function registerMicrosoftTools(server: McpServer, env: Env): void {
  server.registerTool(
    ...
    async ({ query, include_groups }) => {
      try {
        await requireCapability(env, "directory.read");
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        await requireMailbox(env, input.mailbox, "read");
        const ctx = await createMicrosoftContext(env);
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        await requireMailbox(env, input.mailbox, "write");
        const ctx = await createMicrosoftContext(env);
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        await assertKnowledgeSearchAllowed(env);
        ...
        const filtered = await filterKnowledgeItems(env.EL_BUSINESS_DATA, getRequestActor(), result.results);
        return jsonTool({
          ...
          classificationHiddenCount: filtered.hidden,
          engineerKnowledgeOnly: filtered.engineerOnly,
    ...
    async (input) => {
      try {
        ...
        if (input.list_children || !input.item_id) {
          const items = await listFolder(ctx.graph, ctx.policy, {
            driveId: input.drive_id,
            itemId: input.item_id,
          });
          const filtered = await filterKnowledgeItems(
            env.EL_BUSINESS_DATA,
            getRequestActor(),
            (items as Array<{ id?: string; name?: string; path?: string; webUrl?: string }>).map((item) => ({
              ...item,
              driveId: input.drive_id,
            }))
          );
          return jsonTool({ items: filtered.visible, classificationHiddenCount: filtered.hidden });
        }
        const file = await getFile(ctx.graph, ctx.policy, {
            driveId: input.drive_id,
            itemId: input.item_id,
            includeContent: input.include_content,
          });
        const filtered = await filterKnowledgeItems(env.EL_BUSINESS_DATA, getRequestActor(), [
          {
            id: input.item_id,
            driveId: input.drive_id,
            name: (file as { name?: string }).name ?? null,
            path: (file as { path?: string }).path ?? null,
            webUrl: (file as { webUrl?: string }).webUrl ?? null,
          },
        ]);
        if (filtered.visible.length === 0) {
          throw new AuthorizationError(
            "This document is not classified for the caller's role.",
            "EL_RBAC_DENIED",
            403,
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        await requireCalendar(env, input.mailbox, "read");
        const ctx = await createMicrosoftContext(env);
        if (input.event_id) {
          return jsonTool({
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        await requireCalendar(env, input.mailbox, "write");
        const ctx = await createMicrosoftContext(env);
...
export async function searchCompanyKnowledgeViaMicrosoft(
  ...
  return {
    ...
    note: "Company knowledge index (R2/Vectorize) is not provisioned. Results come from the bounded Microsoft file catalogue (protected users excluded before indexing) plus optional Graph search, then EL RBAC classification.",
    confidence: filtered.visible.length ? "plausible" : "weak",
    excludedProtectedCount: result.excludedProtectedCount,
    classificationHiddenCount: filtered.hidden,
    engineerKnowledgeOnly: filtered.engineerOnly,
    catalogue: result.catalogue,
...
async function assertKnowledgeSearchAllowed(env: Env): Promise<void> {
  const company = await authorize(env, "knowledge.company.read");
  const engineer = await authorize(env, "knowledge.engineer.read");
  if (company.allowed || engineer.allowed) return;
  throw new AuthorizationError(
    engineer.reason,
    "EL_RBAC_DENIED",
    403,
