-- Chunk-level metadata (page, section, heading) and requires_ocr document status
...
CREATE TABLE knowledge_documents_new (
  ...
  mime_type TEXT,
  byte_size INTEGER,
  status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'indexed', 'failed', 'requires_ocr')),
  metadata TEXT,
...
INSERT INTO knowledge_chunks_new (
  id, document_id, chunk_index, content, vector_id, token_estimate, metadata, created_at
)
