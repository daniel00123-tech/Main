...
def run(args: argparse.Namespace) -> int:
    ...
    fallback_category = category_by_name.get(normalise_name(args.fallback_category)) if args.fallback_category else None
    ...
    for job_id in sorted(jobs_by_id):
        ...
        if is_uncategorised(job):
            try:
                ...
                category_reason = "creator from {source} matches an existing category"
                category_source = "creator"
                if creator and not target_category and fallback_category:
                    target_category = fallback_category
                    category_reason = (
                        "creator from {source} does not match an existing category; "
                        f"using fallback category {fallback_category['label']!r}"
                    )
                ...
                if creator and target_category:
                    intended.append(
                        IntendedUpdate(
                            ...
                            reason="uncategorised job; " + category_reason.format(source=source),
                            params={
                                "action": "JobSave",
                                "JobId": job_id,
                    ...
                    skip_reasons.append(f"uncategorised but could not identify creator: {source}")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Existing category to use when an uncategorised job creator has no matching category.",
    )
