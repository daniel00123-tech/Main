    ...
    for job_id in sorted(jobs_by_id):
        ...
        if is_uncategorised(job):
            try:
                ...
                category_reason = "matches an existing category"
                if creator and not target_category and fallback_category:
                    target_category = fallback_category
                    category_reason = f"does not match an existing category; using confirmed fallback category {FALLBACK_CATEGORY}"
                ...
                if creator and target_category:
                    intended.append(
                        IntendedUpdate(
                            ...
                            reason=f"uncategorised job; creator from {source} {category_reason}",
                            params={
                                "action": "JobSave",
                                "JobId": job_id,
                            ...
                            target={
                                ...
                                "creator": creator,
                                "fallbackCategoryUsed": matching_category is None,
                            },
                        )
                    )
                    ...
                    skip_reasons.append(
                        f"uncategorised but no matching category for creator and fallback category is not confirmed: {creator}"
                    )
                else:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--days", type=int, default=30, help="Look back this many rolling days from the run start.")
