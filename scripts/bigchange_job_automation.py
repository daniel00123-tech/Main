...
def run(args: argparse.Namespace) -> int:
    ...
    for job_id in sorted(jobs_by_id):
        ...
        if is_uncategorised(job):
            try:
                ...
                matched_category = category_by_name.get(normalise_name(creator)) if creator else None
                target_category = matched_category or (fallback_category if creator else None)
                if target_category:
                    if matched_category:
                        ...
                        reason = (
                            ...
                            f"using confirmed {FALLBACK_CATEGORY} fallback"
                        )
                    intended.append(
                        IntendedUpdate(
                            job_id=job_id,
                            ...
                            target={
                                ...
                                "creator": creator,
                                "fallback": matched_category is None,
                            },
                        )
                    )
    ...
    preview_payload = {
        ...
        "confirmed_references": {
            ...
            "fallbackCategory": (
                {"name": fallback_category.get("label"), "id": fallback_category.get("id")}
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--days", type=int, default=30, help="Look back this many days from the run timestamp.")
