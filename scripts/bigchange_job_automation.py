...
def run(args: argparse.Namespace) -> int:
    ...
    for job_id in sorted(jobs_by_id):
        ...
        if is_uncategorised(job):
            try:
                ...
                target_category = category_by_name.get(normalise_name(creator)) if creator else None
                category_source = "creator"
                if creator and not target_category:
                ...
                if creator and target_category:
                    intended.append(
                        IntendedUpdate(
                            ...
                            reason=(
                                f"uncategorised job; creator from {source} "
                                + (
                                    "matches an existing category"
                                    if category_source == "creator"
                                    else f"has no matching category; using confirmed fallback category {FALLBACK_CATEGORY}"
                                )
                            ),
                            params={
                                "action": "JobSave",
        ...
        if job_id in flagged_ids:
            try:
                if normalise_name(job.get("CurrentFlag")) != normalise_name(AUTO_CLOSE_DOWN):
                    skip_reasons.append("Auto Close Down tag matched but CurrentFlag is not Auto Close Down")
                else:
    ...
    preview_payload = {
        ...
        "confirmed_references": {
            ...
            "fallbackCategory": fallback_category.get("label"),
