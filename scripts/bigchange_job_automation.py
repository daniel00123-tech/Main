...
def build_summary(
    *,
    ...
    successful_operations = [r for r in apply_results if r.get("status") == "updated"]
    failed_operations = [r for r in apply_results if r.get("status") == "failed"]
    jobs_with_apply_results = jobs_with_success | jobs_with_failure
    ...
    lines = [
        ...
        f"- Total updated: {len(jobs_with_success) if not dry_run else 0} jobs ({len(successful_operations) if not dry_run else 0} operations)",
        f"- Total skipped: {total_jobs - len(jobs_with_apply_results) if not dry_run else total_jobs - len(jobs_with_intended)}",
        f"- Total failed: {len(jobs_with_failure) if not dry_run else 0} jobs ({len(failed_operations) if not dry_run else 0} operations)",
        "",
...
def run(args: argparse.Namespace) -> int:
    ...
    if not fallback_category:
        raise BigChangeError(f'Could not confirm fallback Job category named "{FALLBACK_CATEGORY}"')
                    ...
                    intended.append(
                        IntendedUpdate(
                            ...
                            target={
                                ...
                                "creator": creator,
                                "matchedCreatorCategory": bool(matching_category),
                            },
                        )
                    )
