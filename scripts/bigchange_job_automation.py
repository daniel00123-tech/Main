...
AUTO_CLOSE_ACTIONED_NOTE = "Automated Auto Close Down actioned update"
...
def build_summary(
    *,
    ...
    successful_operations = sum(1 for row in apply_results if row.get("status") == "updated")
    failed_operations = sum(1 for row in apply_results if row.get("status") == "failed")
    ...
    lines = [
        ...
        f"- Total skipped: {total_jobs - len(jobs_with_apply_result) if not dry_run else total_jobs - len(jobs_with_intended)}",
        f"- Total failed: {len(jobs_with_failure) if not dry_run else 0}",
        f"- Total update operations previewed: {len(intended_updates)}",
        f"- Total update operations applied successfully: {successful_operations if not dry_run else 0}",
        f"- Total update operations failed: {failed_operations if not dry_run else 0}",
        "",
    ...
    for job_id in sorted(jobs_by_id):
        ...
        if is_uncategorised(job):
            try:
                ...
                if creator and not target_category and fallback_category:
                    target_category = fallback_category
                    category_reason = (
                        f"uncategorised job; creator from {source} has no existing category; "
                        f"fallback category {FALLBACK_CATEGORY!r} confirmed"
                    )
                ...
                if creator and target_category:
                    intended.append(
                        IntendedUpdate(
                            ...
                            target={
                                ...
                                "usedFallback": target_category is fallback_category and matching_category is None,
                            },
                        )
                    )
                    ...
                    skip_reasons.append(
                        f"uncategorised but no matching category for creator and fallback category not confirmed: {creator}"
                    )
                else:
            ...
            try:
                ...
                if is_actioned(job) and invoice_created:
                    ...
                    if not invoice_created:
                        ...
                        skip_reasons.append("Auto Close Down invoice status already InvoiceCreated")
            except Exception as exc:  # noqa: BLE001 - keep processing remaining jobs.
