...
HAYLEY_LONGFORD = "Hayley Longford"
...
def run(args: argparse.Namespace) -> int:
    ...
    fallback_category = category_by_name.get(normalise_name(HAYLEY_LONGFORD))
        ...
        if is_uncategorised(job):
            try:
                ...
                if creator and target_category:
                    direct_match = target_category is matching_category
                    reason = (
                        f"uncategorised job; creator from {source} matches an existing category"
                        if direct_match
                        else (
                            f"uncategorised job; creator from {source} has no existing matching category; "
                            f"fallback category {HAYLEY_LONGFORD!r} confirmed"
                        )
                    )
                    intended.append(
                        IntendedUpdate(
                            ...
                            target={
                                ...
                                "JobCategoryId": target_category.get("id") or target_category.get("Id"),
                                "creator": creator,
                                "assignment": "direct_creator_match" if direct_match else "fallback",
                            },
                        )
                    )
                    ...
                    if creator:
                        skip_reasons.append(
                            f"uncategorised but no matching category for creator and fallback unavailable: {creator}"
                        )
                    else:
                        skip_reasons.append(f"uncategorised but creator unavailable: {source}")
    ...
    preview_payload = {
        ...
        "confirmed_references": {
            ...
            "fallbackJobCategory": (
                {
                    "label": fallback_category.get("label"),
                    "id": fallback_category.get("id") or fallback_category.get("Id"),
                }
