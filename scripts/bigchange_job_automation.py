...
def run(args: argparse.Namespace) -> int:
                ...
                if creator and target_category:
                    if matching_category:
                        ...
                        reason = (
                            ...
                            "using confirmed Hayley Longford fallback"
                        )
                    intended.append(
                        IntendedUpdate(
                            job_id=job_id,
                    ...
                    if creator:
                        skip_reasons.append(
                            f"uncategorised but no matching category for creator and Hayley Longford fallback missing: {creator}"
                        )
                    else:
    ...
    preview_payload = {
        ...
        "confirmed_references": {
            ...
            "fallbackJobCategory": fallback_category.get("label") if fallback_category else None,
            "fallbackJobCategoryId": fallback_category.get("id") if fallback_category else None,
        },
