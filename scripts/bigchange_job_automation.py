...
def build_summary(
    *,
    ...
    jobs_touched = jobs_with_success | jobs_with_failure
    ...
    lines = [
        ...
        f"- Total skipped: {max(total_jobs - len(jobs_touched), 0) if not dry_run else total_jobs - len(jobs_with_intended)}",
...
def run(args: argparse.Namespace) -> int:
    ...
    for job_id in sorted(jobs_by_id):
        ...
        if is_uncategorised(job):
            try:
                ...
                category_reason = f"uncategorised job; creator from {source} matches an existing category"
                if creator and not target_category and fallback_category:
                    target_category = fallback_category
                    category_reason = (
                        f"uncategorised job; creator from {source} ({creator}) has no matching category; "
                        f"using confirmed fallback category {UNCATEGORISED_FALLBACK_CATEGORY}"
                    )
                if creator and target_category:
                    intended.append(
                        IntendedUpdate(
                            ...
                            target={"Category": target_category["label"], "JobCategoryId": target_category.get("id"), "creator": creator},
                        )
                    )
                    ...
                    if not fallback_category:
                        skip_reasons.append(
                            f"uncategorised but fallback category {UNCATEGORISED_FALLBACK_CATEGORY} was not confirmed"
                        )
                    else:
                        skip_reasons.append(f"uncategorised but no creator could be confirmed: {creator or source}")
                    ...
                    if not is_actioned(job):
                        intended.append(
                            IntendedUpdate(
                                ...
                                reason="Auto Close Down flag confirmed; mark job actioned",
                                params={
    ...
    preview_payload = {
        ...
        "confirmed_references": {
            ...
            "fallbackJobCategory": fallback_category["label"] if fallback_category else None,
            "fallbackJobCategoryConfirmed": bool(fallback_category),
