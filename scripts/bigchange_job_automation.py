...
def has_current_auto_close_down_flag(job: dict[str, Any]) -> bool:
...
def build_summary(
    *,
    ...
    operation_success_count = sum(1 for row in apply_results if row.get("status") == "updated")
    operation_failure_count = sum(1 for row in apply_results if row.get("status") == "failed")
    ...
    lines = [
        ...
        f"- Total update operations previewed: {len(intended_updates)}",
        f"- Total update operations attempted: {len(apply_results) if not dry_run else 0}",
        f"- Total update operations succeeded: {operation_success_count if not dry_run else 0}",
        f"- Total update operations failed: {operation_failure_count if not dry_run else 0}",
        "",
...
def run(args: argparse.Namespace) -> int:
    ...
    for job_id in sorted(jobs_by_id):
        ...
        if is_uncategorised(job):
            try:
                ...
                if creator and target_category:
                    reason = (
                        f"uncategorised job; creator from {source} matches an existing category"
                        if matching_category
                        else f"uncategorised job; creator from {source} has no matching category; using confirmed fallback category"
                    )
                    intended.append(
                        IntendedUpdate(
                            job_id=job_id,
                            ...
                            target={
                                ...
                                "creator": creator,
                                "fallback": not bool(matching_category),
                            },
                        )
                    )
                    ...
                    if creator:
                        skip_reasons.append(f"uncategorised but no matching category for creator and fallback category not confirmed: {creator}")
                    else:
                        skip_reasons.append(f"uncategorised but creator could not be confirmed: {source}")
        ...
        if job_id in flagged_ids:
            try:
                if not has_current_auto_close_down_flag(job):
                    skip_reasons.append(f"Auto Close Down tag matched but CurrentFlag is not Auto Close Down: {job.get('CurrentFlag')}")
                else:
                        ...
                        if not invoice_created:
                            intended.append(
                                IntendedUpdate(
                                    ...
                                    before={"InvoiceCreated": invoice_created, "CurrentFlag": job.get("CurrentFlag")},
    ...
    preview_payload = {
        ...
        "confirmed_references": {
            ...
            "fallbackCategory": fallback_category.get("label") if fallback_category else None,
