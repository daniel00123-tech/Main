...
LOOKBACK_DAYS = 14
SEARCH_DAYS = 14
SETTLED_VERIFICATION_DELAY_SECONDS = 5
...
def update_record_from_job_state(
    record: dict[str, Any],
    job: dict[str, Any],
    resource: dict[str, Any],
    *,
    ...
    if not planned:
        return record
    duration = parse_duration(job.get("Duration")) or as_int(record.get("duration_minutes"), default=60) or 60
    planned_end = parse_datetime(job.get("PlannedEnd")) or planned + dt.timedelta(minutes=duration)
    record.update(
        {
            "resource": resource_label(resource),
            "resource_id": resource_id(resource),
            "scheduled_date": planned.date().isoformat(),
            "start": planned.strftime("%H:%M"),
            "end": planned_end.strftime("%H:%M"),
            "duration_minutes": int((planned_end - planned).total_seconds() // 60) or duration,
            "verification": verification,
        }
    )
    return record
...
def reconcile_settled_state(
    ...
    *,
    ...
    """Re-read applied jobs after BigChange settles and repair dropped resources."""
    if not apply or not applied:
        return []

    time.sleep(SETTLED_VERIFICATION_DELAY_SECONDS)
    ...
    for record in applied:
        ...
        if not job_id:
            failures.append({"ref": job_ref, "site": record.get("site"), "error": "Applied record has no job_id", "mode": "settled_verify"})
            continue

        job = fetch_job_by_id(client, job_id)
        planned = parse_datetime(job.get("PlannedStart")) if job else None
        resource_name = str(job.get("Resource") or "") if job else ""
        actual_resource = by_name.get(normalise(resource_name))

        if job and planned and not resource_name:
            intended = by_id.get(as_int(record.get("resource_id"), default=0) or 0)
            if not intended:
                failures.append(
                    {
                        "ref": job_ref,
                        "site": record.get("site"),
                        "error": "Settled state dropped resource and intended resource could not be resolved",
                        "mode": "settled_verify",
                    }
                )
                continue
                ...
                failures.append(
                    {
                        "ref": job_ref,
                        "site": record.get("site"),
                        "error": "Settled state dropped resource and no repair slot was available",
                        "mode": "settled_verify",
                    }
                )
                continue
            repair_day, repair_start, repair_end, _before, _after = slot
            repair_day, repair_start, repair_end, repair_message = schedule_with_verification(
                client,
                rules,
                job_id=job_id,
                job_ref=job_ref,
                resource=intended,
                scheduled_day=repair_day,
                start=repair_start,
                end=repair_end,
            ...
            record.update(
                {
                    "scheduled_date": repair_day.isoformat(),
                    "start": repair_start.strftime("%H:%M"),
                    "end": repair_end.strftime("%H:%M"),
                    "verification": f"settled repair: {repair_message}",
                }
            )

        if not job or not planned or not resource_name:
            failures.append(
                {
                    "ref": job_ref,
                    "site": record.get("site"),
                    "error": "Settled verification failed: job missing PlannedStart or Resource",
                    "mode": "settled_verify",
                }
            )
            continue
            ...
            failures.append(
                {
                    "ref": job_ref,
                    "site": record.get("site"),
                    "error": f"Settled resource '{resource_name}' was not found in Resources",
                    "mode": "settled_verify",
                }
            )
            continue

        if not is_site_role_resource(actual_resource, rules):
            failures.append(
                {
                    "ref": job_ref,
                    "site": record.get("site"),
                    "error": f"Settled resource '{resource_name}' is not an active site-based Tech/CT/HK resource",
                    "mode": "settled_verify",
                }
            )
            continue

        verified, message = verify_schedule(client, job_id, job_ref, resource_id(actual_resource), planned.date())
        if not verified:
            failures.append({"ref": job_ref, "site": record.get("site"), "error": f"Settled verification failed: {message}", "mode": "settled_verify"})
            continue

        before = dict(record)
        update_record_from_job_state(record, job, actual_resource, verification=f"settled {message}")
        if any(record.get(key) != before.get(key) for key in ("resource", "resource_id", "scheduled_date", "start", "end", "duration_minutes")):
            correction = dict(record)
            correction.update(
                {
                    "timestamp": utc_now().isoformat(),
                    "mode": "daily_settled_correction",
                    "previous_resource": before.get("resource"),
                    "previous_resource_id": before.get("resource_id"),
                    "previous_scheduled_date": before.get("scheduled_date"),
                    "previous_start": before.get("start"),
                    "previous_end": before.get("end"),
                    "previous_duration_minutes": before.get("duration_minutes"),
                }
            )
            append_audit(correction)
...
def main() -> int:
    ...
    failed.extend(reconcile_settled_state(client, rules, applied, resources, apply=args.apply))
