...
UNSAFE_DOCUMENT_TYPE_TERMS = ("purchase", "credit", "order", "quote", "pro forma", "proforma")
...
def document_is_processable(doc: dict[str, Any]) -> tuple[bool, str]:
    ...
    if any(contains_keyword(norm_doc_type, term) for term in UNSAFE_DOCUMENT_TYPE_TERMS):
        return False, f"document type is {doc_type}"
    if norm_doc_type not in SALES_INVOICE_DOCUMENT_TYPES and compact_doc_type not in SALES_INVOICE_DOCUMENT_TYPES:
        return False, f"document type is {doc_type}"
...
class TempInvoiceNominalCorrector:
    ...
    def identify_job(self, doc: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
        ...
        job, resolved_job_id = self.resolve_job(job_id, job_ref)
        ...
        if group_job:
            ...
            job, resolved_job_id = self.resolve_job(group_job_id, group_job_ref)
            if job and resolved_job_id:
                return job, resolved_job_id
            if group_job_id:
    ...
    def resolve_job(self, job_id: str, job_ref: str) -> tuple[dict[str, Any] | None, str | None]:
        ...
        if job is None and job_ref:
            ...
            resolved_job_id = clean_text(first_present(job or {}, ("JobId", "JobID", "Id", "ID"))) or resolved_job_id
