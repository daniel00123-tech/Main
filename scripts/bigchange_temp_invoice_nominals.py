...
REJECTED_FINANCIAL_DOC_TYPE_KEYWORDS = ("credit", "purchase", "order", "quote")
...
def document_is_processable(doc: dict[str, Any]) -> tuple[bool, str]:
    ...
    if any(keyword in norm_doc_type for keyword in REJECTED_FINANCIAL_DOC_TYPE_KEYWORDS):
        return False, f"document type is {doc_type}"
    if norm_doc_type not in ALLOWED_FINANCIAL_DOC_TYPES:
        return False, f"document type is {doc_type}"
    ...
    if document_has_sync_marker(doc):
        return False, "document is already synchronised"
    return True, ""


def document_has_sync_marker(doc: dict[str, Any]) -> bool:
    for field_name in ("SyncDate", "SynchronizedDate", "SynchronisedDate", "SynchronizationDate", "SynchronisationDate"):
        if is_populated(first_present(doc, (field_name,))):
            return True
    for field_name in ("IsSynchronized", "IsSynchronised", "Synchronized", "Synchronised"):
        if is_truthy_flag(first_present(doc, (field_name,))):
            return True
    return False
...
class TempInvoiceNominalCorrector:
    ...
    def identify_job(self, doc: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
        ...
        if job is None:
            ...
            job_id = group_job_id or clean_text(first_present(job or {}, ("JobId", "JobID", "Id", "ID"))) or job_id
        else:
    ...
    def verify_document(
        ...
        if verified_doc_id != clean_text(expected_doc_id):
            raise RuntimeError(f"verification returned DocId {verified_doc_id or '<missing>'}, expected {expected_doc_id}")
