...
SAFE_INVOICE_DOC_TYPES = {"invoice", "sales invoice", "salesinvoice", "si"}
UNSAFE_DOC_TYPE_KEYWORDS = ("purchase", "credit", "order", "quote")
...
def document_is_processable(doc: dict[str, Any]) -> tuple[bool, str]:
    ...
    if any(keyword in norm_doc_type for keyword in UNSAFE_DOC_TYPE_KEYWORDS):
        return False, f"document type is {doc_type}"
    if norm_doc_type not in SAFE_INVOICE_DOC_TYPES:
        return False, f"document type is {doc_type}"
...
class TempInvoiceNominalCorrector:
    ...
    def identify_job(self, doc: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
        ...
        job, resolved_job_id = self.lookup_job(job_id=job_id, job_ref=job_ref)
        if job:
        ...
        if group_job:
            ...
            job, resolved_job_id = self.lookup_job(job_id=group_job_id, job_ref=group_job_ref)
            if job:
                return job, resolved_job_id
            return group_job, group_job_id or None

        return None, None

    def lookup_job(self, *, job_id: str | None = None, job_ref: str | None = None) -> tuple[dict[str, Any] | None, str | None]:
