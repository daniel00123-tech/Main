...
def is_truthy_marker(value: Any) -> bool:
    ...
    return text not in {"", "none", "null", "0", "false", "no", "n"}
...
def document_is_processable(doc: dict[str, Any]) -> tuple[bool, str]:
    ...
    for field_name in SYNC_FLAG_FIELDS:
        if is_truthy_marker(first_present(doc, (field_name,))):
            return False, f"{field_name} indicates the document is synchronised"
...
class TempInvoiceNominalCorrector:
    ...
    def identify_job(self, doc: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
        ...
        return self.resolve_job(job_id=group_job_id, job_ref=group_job_ref)

    def resolve_job(self, *, job_id: str | None, job_ref: str | None) -> tuple[dict[str, Any] | None, str | None]:
    ...
    def verify_document(
        ...
        for original, verified in zip(original_lines, verified_lines, strict=True):
            ...
            if clean_text(original.tax_code) and clean_text(original.tax_code) != clean_text(verified.tax_code):
                if not (clean_text(original.tax_rate) and decimal_equal(original.tax_rate, verified.tax_rate)):
