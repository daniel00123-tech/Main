...
def document_is_processable(doc: dict[str, Any]) -> tuple[bool, str]:
    ...
    if any(fragment in norm_doc_type for fragment in ("credit", "purchase", "order")):
        return False, f"document type is {doc_type}"
    if norm_doc_type != "si" and "invoice" not in norm_doc_type:
        return False, f"document type is {doc_type}"
...
class TempInvoiceNominalCorrector:
    ...
    def identify_job(self, doc: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
        ...
        if job is None:
            job, job_id = self.resolve_group_job(doc)
        return job, job_id or None

    def resolve_group_job(self, doc: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
        ...
        if group_job_id:
            return self.client.job(job_id=group_job_id) or group_job, group_job_id
