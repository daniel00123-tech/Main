...
def category_lookup_id(row: dict[str, Any], fields: tuple[str, ...] = CATEGORY_LOOKUP_ID_FIELDS) -> str:
    return clean_name(first_present(row, fields))


def category_name_from_category_row(row: dict[str, Any]) -> str:
    return clean_name(first_present(row, JOB_CATEGORY_NAME_FIELDS + ("label", "Name")))


def build_category_lookup(category_rows: list[dict[str, Any]]) -> dict[str, str]:
    category_by_id: dict[str, str] = {}
    for category in category_rows:
        name = category_name_from_category_row(category)
        category_id = category_lookup_id(category)
        if name and category_id:
            category_by_id[category_id] = name
    return category_by_id


def resolve_job_category_name(row: dict[str, Any], category_by_id: dict[str, str] | None = None) -> str:
    category = job_category_name(row)
    if category:
        return category
    if not category_by_id:
        return ""
    category_id = category_lookup_id(row, JOB_CATEGORY_ID_FIELDS)
    if not category_id:
        return ""
...
def add_items(
    ...
    for row in rows:
        category = resolve_job_category_name(row, category_by_id)
...
def build_report(client: BigChangeClient, freshdesk_client: FreshdeskClient) -> dict[str, Any]:
    ...
    category_by_id = build_category_lookup(category_rows)
    for category in category_rows:
        name = category_name_from_category_row(category)
    ...
    add_items(grouped, staff_names, unallocated_rows, "unallocated_jobs", CREATED_DATE_FIELDS, today, category_by_id)
    ...
    add_items(grouped, staff_names, historic_rows, "historic_jobs", PLANNED_START_FIELDS, today, category_by_id)
    ...
    add_items(grouped, staff_names, uninvoiced_rows, "uninvoiced_jobs", STATUS_DATE_FIELDS, today, category_by_id)
    ...
    add_items(grouped, staff_names, unactioned_rows, "unactioned_jobs", STATUS_DATE_FIELDS, today, category_by_id)
