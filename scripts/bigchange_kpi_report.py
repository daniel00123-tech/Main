...
def job_category_name(row: dict[str, Any], category_names_by_id: dict[str, str] | None = None) -> str:
    direct_name = clean_name(first_present(row, CATEGORY_NAME_FIELDS))
    if direct_name:
        return direct_name
    ...
    if not category_id:
        return ""
    return clean_name(category_names_by_id.get(category_id))
...
def build_report(client: BigChangeClient, freshdesk_client: FreshdeskClient) -> dict[str, Any]:
    ...
    category_names_by_id = category_lookup(categories)
