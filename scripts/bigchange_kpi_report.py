...
def category_id_from_category_row(row: dict[str, Any]) -> str:
    return clean_name(first_present(row, ("Id", "ID", "CategoryId", "CategoryID", "JobCategoryId", "JobCategoryID")))
...
def job_category_name(row: dict[str, Any], category_lookup: dict[str, str] | None = None) -> str:
    ...
    if name or not category_lookup:
        return name
    return category_lookup.get(category_id(row), "")
...
def build_report(client: BigChangeClient, freshdesk_client: FreshdeskClient) -> dict[str, Any]:
    ...
    for category in client.categories():
        ...
        if not should_exclude_category(name):
            ...
            lookup_id = category_id_from_category_row(category)
            if lookup_id:
                category_lookup[lookup_id] = name
