...
def render_html(report: dict[str, Any]) -> str:
    ...
    totals = {metric_key: sum(row["metrics"][metric_key]["count"] for row in report["staff_rows"]) for metric_key, _label in KPI_ORDER}
    total_sales = format_currency(
        sum((decimal.Decimal(str(row["current_month_sales"])) for row in report["staff_rows"]), DECIMAL_ZERO)
    )
...
<body>
  <main class="dashboard">
    ...
    <section class="total-cards">
      ...
      <div class="total-card"><span>{month_name} sales</span><strong>{total_sales}</strong></div>
    </section>
