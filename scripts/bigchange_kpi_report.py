...
def render_html(report: dict[str, Any]) -> str:
    ...
    for idx, row in enumerate(report["staff_rows"], start=1):
        ...
        row_class = ' class="escalated"' if row["red_kpis"] else ""
        escalation_icon = '<span class="escalation-icon">!</span>' if row["red_kpis"] else ""
        cells = [
    ...
    total_sales = sum(decimal.Decimal(str(row["current_month_sales"])) for row in report["staff_rows"])
