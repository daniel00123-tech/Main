...
def build_report(client: BigChangeClient) -> dict[str, Any]:
    ...
    staff_rows.sort(key=ranking_key)
    return {
...
def render_html(report: dict[str, Any]) -> str:
    ...
    sales_total = sum(
        (decimal.Decimal(str(row["current_month_sales"])) for row in report["staff_rows"]),
        DECIMAL_ZERO,
    )
    sales_total_display = html.escape(format_currency(sales_total))
...
<body>
  <main class="dashboard">
    ...
    <section class="total-cards">
      ...
      <div class="total-card"><span>{month_name} sales</span><strong>{sales_total_display}</strong></div>
    </section>
