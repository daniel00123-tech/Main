...
def build_category_lookup(categories: list[dict[str, Any]]) -> dict[str, str]:
    ...
    for category in categories:
        name = clean_name(first_present(category, CATEGORY_NAME_FIELDS + ("label", "Name")))
        category_id = id_key(first_present(category, CATEGORY_RECORD_ID_FIELDS))
...
def job_category_name(row: dict[str, Any], category_lookup: dict[str, str] | None = None) -> str:
    ...
    direct_name_id = id_key(direct_name)
    if category_lookup and direct_name_id in category_lookup:
        return category_lookup[direct_name_id]
    if direct_name and (direct_name_id != direct_name or as_int(direct_name, default=None) is not None):
        return ""
    if direct_name:
        return direct_name

    category_id = id_key(first_present(row, CATEGORY_ID_FIELDS))
    if category_id and category_lookup:
