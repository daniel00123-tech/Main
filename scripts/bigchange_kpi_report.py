...
JOB_CATEGORY_NAME_FIELDS = ("Category", "CategoryName", "JobCategory", "JobCategoryName", "JobCategoryLabel")
JOB_CATEGORY_ID_FIELDS = ("CategoryId", "CategoryID", "JobCategoryId", "JobCategoryID")
CATEGORY_NAME_FIELDS = ("label", "JobCategoryName", "CategoryName", "Name")
CATEGORY_ID_FIELDS = ("Id", "ID", "CategoryId", "CategoryID", "JobCategoryId", "JobCategoryID")
...
def job_category_name(row: dict[str, Any], category_lookup: dict[str, str] | None = None) -> str:
    direct_name = clean_name(first_present(row, JOB_CATEGORY_NAME_FIELDS))
    if category_lookup and direct_name in category_lookup:
        return category_lookup[direct_name]
    if direct_name and not direct_name.isdigit():
        return direct_name

    category_id = clean_name(first_present(row, JOB_CATEGORY_ID_FIELDS)) or direct_name
...
def build_report(client: BigChangeClient, freshdesk_client: FreshdeskClient) -> dict[str, Any]:
    ...
    for category in client.categories():
        ...
        if not should_exclude_category(name):
            ...
            if identifier:
                category_lookup[identifier] = name
