...
def build_category_lookup(categories: list[dict[str, Any]]) -> dict[str, str]:
    ...
    for category in categories:
        ...
        if not name:
            continue
        for field in ("Id", "ID", *CATEGORY_ID_FIELDS):
            value = first_present(category, (field,))
            if value not in (None, ""):
                lookup[category_lookup_key(value)] = name
    return lookup
...
def job_category_name(row: dict[str, Any], category_lookup: dict[str, str] | None = None) -> str:
    ...
    if explicit_name:
        if category_lookup:
            mapped_name = category_lookup.get(category_lookup_key(explicit_name))
            if mapped_name:
                return mapped_name
    ...
    if category_lookup:
        ...
        if category_id:
            return category_lookup.get(category_lookup_key(category_id), "")
    return ""
