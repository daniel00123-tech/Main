...
def staff_ranking_key(row: dict[str, Any]) -> tuple[int, int, int, str]:
    return (
        int(row["red_kpis"]),
        int(row["amber_kpis"]),
...
def build_report(client: BigChangeClient) -> dict[str, Any]:
    ...
    staff_rows.sort(key=staff_ranking_key)
    return {
    ...
    <header>
      <div class="brand">
        ...
        <div>
          ...
          <div class="sub">Generated {report_date} - jobs from {job_lookback_start} onwards, grouped by BigChange job category staff owner</div>
        </div>
      </div>
      <div class="summary">
