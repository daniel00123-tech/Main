...
def render_html(report: dict[str, Any]) -> str:
    ...
    total_sales = sum((decimal.Decimal(str(row["current_month_sales"])) for row in report["staff_rows"]), DECIMAL_ZERO)
    total_sales_display = html.escape(format_currency(total_sales))
...
<body>
  <main class="dashboard">
    ...
    <section class="total-cards">
      ...
      <div class="total-card"><span>{month_name} sales</span><strong>{total_sales_display}</strong></div>
    </section>
        ...
        baseline_rows.append(
            {
                ...
                "counts": {metric_key: row["metrics"][metric_key]["count"] for metric_key, _ in JOB_KPI_ORDER},
                "statuses": {metric_key: row["metrics"][metric_key]["status"] for metric_key, _ in JOB_KPI_ORDER},
                "oldest_age_days": {
                    metric_key: row["metrics"][metric_key]["oldest_age_days"] for metric_key, _ in JOB_KPI_ORDER
                },
