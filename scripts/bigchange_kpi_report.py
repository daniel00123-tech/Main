...
def job_category_id(row: dict[str, Any]) -> str:
    return clean_name(first_present(row, CATEGORY_ID_FIELDS))


def category_lookup_from_rows(rows: list[dict[str, Any]]) -> tuple[set[str], dict[str, str]]:
    staff_names: set[str] = set()
    category_lookup: dict[str, str] = {}
    for row in rows:
        name = clean_name(first_present(row, ("label", "JobCategoryName", "CategoryName", "Name")))
        if should_exclude_category(name):
            continue
        staff_names.add(name)
        for field in ("Id", "ID", "CategoryId", "CategoryID", "JobCategoryId", "JobCategoryID"):
            category_id = clean_name(first_present(row, (field,)))
            if category_id:
                category_lookup[category_id] = name
    return staff_names, category_lookup


def resolved_job_category_name(row: dict[str, Any], category_lookup: dict[str, str]) -> str:
    explicit_name = job_category_name(row)
    if explicit_name:
        return explicit_name
    category_id = job_category_id(row)
    if category_id:
...
def add_items(
    ...
    for row in rows:
        category = resolved_job_category_name(row, category_lookup)
...
def build_report(client: BigChangeClient, freshdesk_client: FreshdeskClient) -> dict[str, Any]:
    ...
    staff_names, category_lookup = category_lookup_from_rows(client.categories())
