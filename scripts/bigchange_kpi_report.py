...
URBAN_TO_EMAIL = "urban@nirvana-maintenance.co.uk"
...
def validate_report_recipients(to_email: str, cc_email: str = "") -> None:
    to_addresses = [addr.strip().lower() for addr in to_email.split(",") if addr.strip()]
    if to_addresses != [URBAN_TO_EMAIL]:
        raise ConfigError(f"Urban KPI report must be sent only to {URBAN_TO_EMAIL}")
    cc_addresses = [addr.strip().lower() for addr in cc_email.split(",") if addr.strip()]
    blocked = {"core@nirvana-maintenance.co.uk", "btr@nirvana-maintenance.co.uk"}
    if any(addr in blocked for addr in [*to_addresses, *cc_addresses]):
        raise ConfigError("Refusing to send Urban KPI report to another company's mailbox")
  ...
  <main class="dashboard">
    <header>
      <div class="brand">
        ...
        <div>
          <h1>Urban BigChange KPI Overview</h1>
