...
def category_display_name(row: dict[str, Any]) -> str:
    return clean_name(first_present(row, CATEGORY_NAME_FIELDS))


def category_lookup(categories: list[dict[str, Any]]) -> dict[str, str]:
    lookup: dict[str, str] = {}
    for category in categories:
        identifier = category_identifier(category)
...
def add_items(
    ...
    for row in rows:
        category = resolved_job_category_name(row, categories_by_id)
...
def build_report(client: BigChangeClient, freshdesk_client: FreshdeskClient) -> dict[str, Any]:
    ...
    categories_by_id = category_lookup(categories)
