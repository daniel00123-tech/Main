...
def category_lookup_id(row: dict[str, Any]) -> str:
    return clean_name(
        first_present(row, ("CategoryId", "CategoryID", "JobCategoryId", "JobCategoryID", "Id", "ID", "Value"))
    )
...
def add_items(
    ...
    for row in rows:
        ...
        if not category and category_lookup:
            category = clean_name(category_lookup.get(job_category_id(row), ""))
