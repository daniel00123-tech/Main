...
def parse_date(value: Any) -> dt.datetime | None:
    ...
    if match:
        return dt.datetime.fromtimestamp(int(match.group(1)) / 1000, tz=dt.timezone.utc).replace(tzinfo=None)
...
def category_id(row: dict[str, Any]) -> str:
    return clean_name(first_present(row, CATEGORY_ID_FIELDS))
...
def job_category_name(row: dict[str, Any], categories_by_id: dict[str, str] | None = None) -> str:
    name = category_name(row)
    if name:
        return name
    if categories_by_id:
        return categories_by_id.get(category_id(row), "")
    return ""
...
class BigChangeClient:
    ...
    def invoices_with_items_by_period(self, start: dt.date, end: dt.date) -> list[dict[str, Any]]:
        payload = self.get("InvoicesWithItemsByPeriod", {"Start": start.isoformat(), "End": end.isoformat()})
        if not code_is_success(payload):
...
def add_items(
    ...
    for row in rows:
        category = job_category_name(row, categories_by_id)
...
def is_invoice_created_activity(activity: dict[str, Any]) -> bool:
    ...
    words = set(normalized_text(label).split())
    return "invoicecreated" in compacted or ("invoice" in words and "created" in words)
...
def calculate_sales(
    ...
    for document in eligible:
        matched_staff = match_staff_name(invoice_created_owner(document, activity_cache), staff_by_key)
...
def build_staff_rows(
    staff_names: set[str],
    grouped: dict[str, dict[str, list[dt.datetime | None]]],
    sales: dict[str, decimal.Decimal],
    today: dt.date,
) -> list[dict[str, Any]]:
...
def build_report(client: BigChangeClient) -> dict[str, Any]:
    ...
    add_items(grouped, staff_names, unallocated_rows, "unallocated_jobs", CREATED_DATE_FIELDS, today, categories_by_id)
    ...
    add_items(grouped, staff_names, historic_rows, "historic_jobs", PLANNED_START_FIELDS, today, categories_by_id)
    ...
    uninvoiced_rows = [
        row for row in uninvoiced_rows if row_status_id(row) in COMPLETED_STATUS_IDS and client_status_id(row) == -34
    ]
    add_items(grouped, staff_names, uninvoiced_rows, "uninvoiced_jobs", STATUS_DATE_FIELDS, today, categories_by_id)
    ...
    add_items(grouped, staff_names, unactioned_rows, "unactioned_jobs", STATUS_DATE_FIELDS, today, categories_by_id)

    sales = calculate_sales(client, staff_names, month_start, today)
    staff_rows = build_staff_rows(staff_names, grouped, sales, today)
...
def send_email(png_path: Path) -> None:
    ...
    root["Subject"] = "Daily KPI Overview Report"
...
<p>Kind regards,<br>Daniel Dwyer</p>"""
    ...
    # The only attachment is the dashboard PNG; JSON and HTML remain local artifacts only.
...
def main() -> int:
    try:
        report = build_report(BigChangeClient())
