...
CATEGORY_LOOKUP_ID_FIELDS = ("Id", "ID", "CategoryId", "CategoryID", "JobCategoryId", "JobCategoryID")
CATEGORY_LOOKUP_NAME_FIELDS = ("label", "Label", "JobCategoryName", "CategoryName", "Name")
...
def category_lookup_from_rows(rows: list[dict[str, Any]]) -> dict[str, str]:
    lookup: dict[str, str] = {}
    for row in rows:
        category_id = clean_name(first_present(row, CATEGORY_LOOKUP_ID_FIELDS))
        category_name = clean_name(first_present(row, CATEGORY_LOOKUP_NAME_FIELDS))
...
def job_category_name(row: dict[str, Any], category_lookup: dict[str, str] | None = None) -> str:
    ...
    if not category_id:
        return ""
    return clean_name(category_lookup.get(category_id, ""))
