...
CATEGORY_NAME_FIELDS = ("Category", "CategoryName", "JobCategory", "JobCategoryName", "JobCategoryLabel")
...
def category_identifier(row: dict[str, Any], include_generic_id: bool = False) -> str:
    fields = CATEGORY_ID_FIELDS + (("Id", "ID") if include_generic_id else ())
    identifier = clean_name(first_present(row, fields))
    if not identifier:
        return ""
    parsed = as_int(identifier, default=None)
    return str(parsed) if parsed is not None else identifier
...
def build_category_lookup(categories: list[dict[str, Any]]) -> dict[str, str]:
    ...
    for category in categories:
        ...
        identifier = category_identifier(category, include_generic_id=True)
...
def job_category_name(row: dict[str, Any], category_lookup: dict[str, str] | None = None) -> str:
    ...
    if category_lookup:
        ...
        if identifier:
            return category_lookup.get(identifier, "")
    return ""
