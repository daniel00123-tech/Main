...
Change STAFF_NAME for another staff automation. Known category IDs stay in
KNOWN_CATEGORY_IDS and rates live in COMMISSION_PROFILES so new staff reuse
this engine and the same email layout.
"""
...
STAFF_NAME = os.environ.get("STAFF_NAME", "Lauren")
...
FLAT_PENALTY = decimal.Decimal("5.00")
MIN_PROFIT = decimal.Decimal("8000")
MIN_MARGIN = decimal.Decimal("25")
FROM_EMAIL = "ella@elvexpropertyservices.com"
TO_EMAIL = "william@elvexpropertyservices.com"
CC_EMAIL = "william@elvexpropertyservices.com"


@dataclass(frozen=True)
class CommissionProfile:
    """Staff-specific rates and penalty rules on the shared sale/margin bands."""

    key: str
    rates: tuple[decimal.Decimal, decimal.Decimal, decimal.Decimal]
    cap_rate: decimal.Decimal
    below_floor_mode: str
    po_only_mode: str
    po_only_rate: decimal.Decimal
    flat_penalty: decimal.Decimal
    minus_cap: decimal.Decimal | None


UK_PROFILE = CommissionProfile(
    key="uk",
    rates=(decimal.Decimal("0.05"), decimal.Decimal("0.075"), decimal.Decimal("0.10")),
    cap_rate=decimal.Decimal("0.10"),
    below_floor_mode="catch_up",
    po_only_mode="percent",
    po_only_rate=decimal.Decimal("0.20"),
    flat_penalty=FLAT_PENALTY,
    minus_cap=COMMISSION_CAP,
)

LAUREN_PROFILE = CommissionProfile(
    key="lauren",
    rates=(decimal.Decimal("0.01"), decimal.Decimal("0.02"), decimal.Decimal("0.03")),
    cap_rate=decimal.Decimal("0.03"),
    below_floor_mode="flat",
    po_only_mode="flat",
    po_only_rate=decimal.Decimal("0.20"),
    flat_penalty=FLAT_PENALTY,
    minus_cap=None,
...
def commission_profile(staff_name: str) -> CommissionProfile:
    return COMMISSION_PROFILES.get(staff_name.strip().lower(), UK_PROFILE)


def _capped_minus(value: decimal.Decimal, profile: CommissionProfile) -> decimal.Decimal:
    quantized = _quantize(value)
    if profile.minus_cap is None:
        return quantized
    return max(-profile.minus_cap, quantized)


def _flat_minus(profile: CommissionProfile) -> decimal.Decimal:
    return _quantize(-abs(profile.flat_penalty))


def _below_floor_minus(
    sale: decimal.Decimal,
    profit: decimal.Decimal,
    floor: decimal.Decimal,
    profile: CommissionProfile,
) -> decimal.Decimal:
    if profile.below_floor_mode == "flat":
        return _flat_minus(profile)
    return _capped_minus(profit - sale * (floor / decimal.Decimal("100")), profile)


def _po_only_minus(po: decimal.Decimal, profile: CommissionProfile) -> decimal.Decimal:
    if profile.po_only_mode == "flat":
        return _flat_minus(profile)
    return _capped_minus(-(po * profile.po_only_rate), profile)


def _positive_commission(
    profit: decimal.Decimal,
    rate: decimal.Decimal,
    profile: CommissionProfile,
) -> decimal.Decimal:
    return _quantize(profit * min(rate, profile.cap_rate))
...
def job_commission(
    ...
    scheme = profile or UK_PROFILE
    profit = sale - po
    if sale <= 0:
        if po > 0:
            return _po_only_minus(po, scheme)
        return DECIMAL_ZERO

    margin = profit / sale * decimal.Decimal("100")
    low, mid, high = scheme.rates

    if sale < decimal.Decimal("2000"):
        if margin < decimal.Decimal("20"):
            return _below_floor_minus(sale, profit, decimal.Decimal("20"), scheme)
        ...
        if margin < decimal.Decimal("42"):
            ...
            rate = high
        return _positive_commission(profit, rate, scheme)

    if sale < decimal.Decimal("5000"):
        if margin < decimal.Decimal("12.5"):
            return _below_floor_minus(sale, profit, decimal.Decimal("12.5"), scheme)
    ...
    if margin < decimal.Decimal("10"):
        return _below_floor_minus(sale, profit, decimal.Decimal("10"), scheme)
    if margin < decimal.Decimal("20"):
        rate = low
...
def attach_commissions(
    ...
    for row in rows:
        commission = job_commission(row["sale"], row["po"], scheme)
...
def _scorecard(totals: dict[str, Any]) -> str:
    """Five equal columns. Labels on one row, figures on the next so amounts
    stay in a straight line on mobile. Tables only — no flex.
    """
    figures = (
        money(totals["profit"]),
        money(totals["sale"]),
...
def build_report(staff_name: str, today: dt.date) -> tuple[str, str, dict[str, Any]]:
    ...
    profile = commission_profile(staff_name)
    rows = attach_commissions(raw_rows, profile)
...
def main() -> int:
    ...
    today = now.date()
    staff_name = STAFF_NAME.strip() or "Lauren"
    try:
