...
UK_RATES = (decimal.Decimal("0.05"), decimal.Decimal("0.075"), decimal.Decimal("0.10"))
LAUREN_RATES = (decimal.Decimal("0.01"), decimal.Decimal("0.02"), decimal.Decimal("0.03"))
...
def is_lauren_profile(staff_name: str | None) -> bool:
    return (staff_name or "").strip().lower() == "lauren"


def lauren_order_penalty(order_size: decimal.Decimal) -> decimal.Decimal:
    """Stepped minus from sale, or PO when there is no sale. No floor catch-up."""
    if order_size < decimal.Decimal("500"):
        return decimal.Decimal("-1.50")
    if order_size < decimal.Decimal("2000"):
        return decimal.Decimal("-5.00")
    if order_size < decimal.Decimal("5000"):
...
def job_commission(
    ...
    Sharon / Ella / default UK: 5% / 7.5% / 10%, floor catch-up, 20% of PO, £250 cap.
    Lauren: same sale bands and margin thresholds at 1% / 2% / 3%. Below-floor and
    PO-only use a stepped minus from order size (sale, or PO when there is no sale).
    """
    lauren = is_lauren_profile(staff_name)
    rate_low, rate_mid, rate_high = LAUREN_RATES if lauren else UK_RATES
    profit = sale - po
    if sale <= 0:
        if po > 0:
            if lauren:
                return lauren_order_penalty(po)
    ...
    if sale < decimal.Decimal("2000"):
        if margin < decimal.Decimal("20"):
            if lauren:
                return lauren_order_penalty(sale)
    ...
    if sale < decimal.Decimal("5000"):
        ...
        if margin < decimal.Decimal("35"):
            ...
            rate = rate_high
        return _quantize(profit * min(rate, rate_high))
...
def attach_commissions(
    ...
    for row in rows:
        commission = job_commission(row["sale"], row["po"], staff_name)
...
def build_report(staff_name: str, today: dt.date) -> tuple[str, str, dict[str, Any]]:
    ...
    rows = attach_commissions(raw_rows, staff_name)
