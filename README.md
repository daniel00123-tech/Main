...
The GitHub Actions workflow `Aquilo BigChange KPI Overview Report` runs this command daily at 07:00 UTC and can also be started manually. Configure the workflow with repository secrets matching the environment variable names in the report script; the workflow persists changes to `automation-memory/kpi-baseline.json` for follow-up automations.
