...
The GitHub Actions workflow `.github/workflows/aquilo-bigchange-kpi-overview-report.yml` runs the dashboard every day at 07:00 UTC and can also be started manually. It maps credentials from repository secrets, renders `reports/bigchange-kpi-dashboard.png`, embeds and attaches that PNG in the email, uploads only the PNG artifact, and commits the latest `automation-memory/kpi-baseline.json` snapshot for follow-up automations.
