...
Job KPIs are limited to jobs from the last 12 months. Current-month sales use invoice and credit-note financial documents from the first through the last day of the current month, attributed by the financial document `OrderCreator` web user. The calculation uses line `NetPrice - VatAmount` to report ex-VAT values and includes both synchronised and unsynchronised documents returned by `InvoicesWithItemsByPeriod`.
