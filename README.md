...
Read-only current-month commission email for one staff member (change only `STAFF_NAME`). Known category IDs: Sharon `132264`, Ella `132225`, Lauren `132263`. Groups appear in the month of their last invoice; totals use full history from 1 May 2026. Sharon / Ella use the UK earn rates; Lauren uses 1% / 2% / 3% and a stepped below-floor / PO-only minus. The script never writes to BigChange.

```sh
STAFF_NAME=Lauren python3 scripts/staff_commission.py
```
