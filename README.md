...
Job KPIs are limited to jobs from the last 12 months. Current-month sales use invoice financial documents from the first of the current month through today. Sales are attributed to the latest linked job customer activity with client status `InvoiceCreated`, matching the activity owner back to the job category staff member. The calculation uses line `NetPrice - VatAmount` to report ex-VAT values for non-cancelled, non-deleted, and non-rejected invoices returned by `InvoicesWithItemsByPeriod`.
