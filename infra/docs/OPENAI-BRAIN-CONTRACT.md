...
| --- | --- | --- | --- |
| **PA** | `portal`, `portal_chat` | Staff / director assistant in Company Portal Chat | OpenAI on allowlisted EL and Caddington, Cloudflare fallback |
| **Request** | `whatsapp` | Customer / business incoming requests | OpenAI on allowlisted EL and Caddington, Cloudflare fallback |
...
- OpenAI **is** the main brain for allowlisted **PA** and **requests**: it plans, picks tools, reads evidence, and writes the user-visible answer.
...
- HT and future tenants stay on Cloudflare until they are explicitly allowlisted. Caddington is allowlisted for PA/request only.
...
Unscoped `resolveBrainPolicy({ companyId: "co_el" })` or `co_caddington` with no channel must keep `useOpenAi=false` and `shadow=true`. Superstack deploy guards that invariant. HT must stay off the allowlist.
