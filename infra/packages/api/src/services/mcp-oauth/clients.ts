...
import { getMcpOAuthClient, insertMcpOAuthClient } from "./store";
...
export async function registerMcpOAuthClient(
...
): Promise<{ client: McpOAuthClient; clientIdIssuedAt: number }> {
  const redirectUris = Array.isArray(body.redirect_uris)
    ? body.redirect_uris.filter((value): value is string => typeof value === "string" && value.trim().length > 0)
    : [];
  if (redirectUris.length === 0) {
    throw new McpOAuthRequestError("invalid_client_metadata", "redirect_uris is required");
  }
  for (const uri of redirectUris) {
    if (!isAllowedRedirectUri(uri)) {
      throw new McpOAuthRequestError("invalid_redirect_uri", `Redirect URI is not allowed: ${uri}`);
    }
  }
  const method =
  ...
  const client = await insertMcpOAuthClient(db, {
...
export async function resolveMcpOAuthClient(
  ...
  const registered = await getMcpOAuthClient(db, clientId);
  if (!registered) return null;
  if (!registered.redirectUris.includes(redirectUri)) return null;
  return registered;
}

async function resolveCimdClient(clientId: string, redirectUri: string): Promise<McpOAuthClient | null> {
  let doc: {
    client_id?: string;
    client_name?: string;
