...
export function platformMicrosoftConfigured(env: Env): boolean {
  ...
  if (platformMultitenantAppEnabled(env)) return true;
  return Boolean(tenantId);
}

export function platformSaaSAppCredentials(env: Env): Omit<MicrosoftAppCredentials, "tenantId" | "authMode"> | null {
  const clientId = trim(env.MICROSOFT_CLIENT_ID);
  const clientSecret = trim(env.MICROSOFT_CLIENT_SECRET);
  if (!clientId || !clientSecret) return null;
  return { clientId, clientSecret, credentialSource: "platform" };
}
