...
export function emailBodyRequired(text: string): boolean {
  ...
  const emailContext = /\b(email|inbox|mailbox|outlook|message|they asking|they want|they said)\b/i.test(text);
  if (!emailContext && !/\bwhat (are|were) they asking\b/i.test(text)) {
    return false;
  }
  return (
