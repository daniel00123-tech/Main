#!/usr/bin/env node
/**
 * Post-payment verifier for Caddington £1 LIVE Stripe acceptance.
 * Run ONLY after Daniel completes a genuine £1 checkout manually.
 * Never prints secrets or card data.
 *
 * Usage:
 *   node scripts/verify-stripe-live-1-gbp-acceptance.mjs
 *   node scripts/verify-stripe-live-1-gbp-acceptance.mjs --checkout-id=<local_checkout_id>
 *   node scripts/verify-stripe-live-1-gbp-acceptance.mjs --amount-cents=100
 */
...
const usageForCheckout = checkoutId
  ? d1Query(
      `SELECT id, tool_name, action, company_id, success, recorded_at
       FROM usage_records
       WHERE company_id = '${COMPANY_ID}' AND recorded_at >= datetime('now', '-1 day')
...
const classification = {
  paymentSucceeded:
    checkout?.status === "credited" || checkout?.status === "paid",
  amountExact: checkout ? Number(checkout.amount_cents) === EXPECTED_AMOUNT_CENTS : false,
  currencyGbp: checkout ? String(checkout.currency ?? "GBP").toUpperCase() === "GBP" : false,
  ...
  ledgerPaidOnce:
    paidLedger.length === 1 && Number(paidLedger[0]?.amount_cents) === EXPECTED_AMOUNT_CENTS,
  ...
  auditTrail: audit.some(
    (a) => a.event_type === "wallet.credited" || a.event_type === "payment.confirmed",
  ),
...
console.log(
  JSON.stringify(
    {
      title: "STRIPE LIVE £1 ACCEPTANCE VERIFICATION",
      checkedAt: new Date().toISOString(),
      expectedAmountGbp: EXPECTED_AMOUNT_CENTS / 100,
